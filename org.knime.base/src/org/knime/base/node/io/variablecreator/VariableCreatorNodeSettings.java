...
final class VariableCreatorNodeSettings implements DefaultNodeSettings {
    ...
    NewFlowVariableSettings[] m_newFlowVariables = new NewFlowVariableSettings[]{ //
        new NewFlowVariableSettings("variable_1", FlowVariableType.STRING, "") //
    };
    ...
    enum FlowVariableType {
            ...
            @Label(value = "String", description = """
                    ...
                    """)
            STRING("str", VariableType.StringType.INSTANCE), //
            @Label(value = "Integer", description = """
                    ...
                    value instead.
                    """)
            INTEGER("int", VariableType.IntType.INSTANCE), //
            @Label(value = "Long", description = """
                    ...
                    """)
            LONG("long", VariableType.LongType.INSTANCE), //
            @Label(value = "Double", description = """
                    ...
                    to zero.
                    """)
            DOUBLE("double", VariableType.DoubleType.INSTANCE), //
            @Label(value = "Boolean", description = """
                    ...
                    """)
            BOOLEAN("bool", VariableType.BooleanType.INSTANCE);

        final String m_oldConfigValue;

        final VariableType<?> m_knimeVariableType;

        FlowVariableType(final String oldConfigValue, final VariableType<?> knimeVariableType) {
            m_oldConfigValue = oldConfigValue;
            m_knimeVariableType = knimeVariableType;
        }
        ...
        void createFromStringAndPush(final FlowVariablePusher pusher, final String name, final String value) {
            createFromStringAndPush(pusher, name, value, this);
        }

        private static void createFromStringAndPush(final FlowVariablePusher pusher, final String name,
            final String value, final FlowVariableType type) {
            switch (type) {
                case BOOLEAN:
                    pusher.push(name, VariableType.BooleanType.INSTANCE, Boolean.parseBoolean(value));
                    break;
                case DOUBLE:
                    pusher.push(name, VariableType.DoubleType.INSTANCE, Double.parseDouble(value));
                    break;
                case INTEGER:
                    pusher.push(name, VariableType.IntType.INSTANCE, Integer.parseInt(value));
                    break;
                case LONG:
                    pusher.push(name, VariableType.LongType.INSTANCE, Long.parseLong(value));
                    break;
                case STRING:
                    pusher.push(name, VariableType.StringType.INSTANCE, value);
                    break;
            }
        }

        boolean canConvert(final String value) {
            return valueCanBeConverted(value, this);
        }

        private static boolean valueCanBeConverted(final String value, final FlowVariableType type) {
            return switch (type) {
                case BOOLEAN, STRING -> true;
                case DOUBLE -> Doubles.tryParse(value) != null;
                case INTEGER -> Ints.tryParse(value) != null;
                case LONG -> Longs.tryParse(value) != null;
            };
        }

        @FunctionalInterface
        interface FlowVariablePusher {
            <T> void push(String name, VariableType<T> type, T value);
        }
    }
