 ...
 */
final class FileTable implements DataTable {

    /** The node logger for this class. */
    private static final NodeLogger LOGGER = NodeLogger.getLogger(FileTable.class);
    ...
    /**
     * Creates a new file table with the structure defined in tableSpec and using the settings in frSettings when the
     * file is read.
     *
     * @param tableSpec a table spec defining the structure of the table to create
     * @param frSettings FileReaderSettings specifying the wheres and hows for reading the ASCII data file
     * @param exec the execution context the progress is reported to; if null, no progress is reported
     */
    FileTable(final DataTableSpec tableSpec, final FileReaderSettings frSettings, final ExecutionContext exec) {
        this(tableSpec, frSettings, createFalseArray(tableSpec.getNumColumns()), exec);
    }
     ...
     * @param tableSpec a table spec defining the structure of the table to create
     * @param frSettings FileReaderSettings specifying the wheres and hows for reading the ASCII data file
     * @param skipColumns array with the element set to true if the corresponding column should be skipped (i.e. read
     *            but not be included in the row). The array must have the length of the 'original' column number (in
     *            the file), the specified table spec is the new one (with less columns).
     * @param exec the execution context the progress is reported to; if null, no progress is reported
     */
    FileTable(final DataTableSpec tableSpec, final FileReaderSettings frSettings, final boolean[] skipColumns,
     ...
     * source for all iterators. Especially if an iterator didn't run to the end of the table, it is required to call
     * this method. Otherwise the file handle is not released until the garbage collector cleans up. A call to
     * <code>next()</code> of any iterator after disposing of the iterator has undefined behavior.
     */
    void dispose() {
    ...
    public FileRowIterator iterator() {
        try {
            synchronized (m_iterators) {
                FileRowIterator i = new FileRowIterator(m_frSettings, m_tableSpec, m_skipColums, m_exec);
                m_iterators.add(new WeakReference<FileRowIterator>(i));
                return i;

            }
        } catch (final InvalidSettingsException | IOException e) {
            LOGGER.error("Error occurred while trying to open a stream" + " to '"
                + m_frSettings.getDataFileLocation().getPath() + "'.", e);
        }
        return null;
    }
     ...
     */
    SettingsStatus getStatusOfSettings(final boolean openDataFile) {
     ...
     * @param openDataFile specifies if we should check the accessibility of the data file.
     */

    void addStatusOfSettings(final SettingsStatus status, final boolean openDataFile) {
     ...
     */
    private void addTableSpecStatusOfSettings(final SettingsStatus status, final DataTableSpec tableSpec) {
        ...
        for (int c = 0; c < tableSpec.getNumColumns(); c++) {
            ...
            if (tableSpec.getColumnSpec(c) == null) {
                status.addError("Column spec for column with index '" + c + "' is not set.");
            } else {
                // check col type
                DataColumnSpec cSpec = tableSpec.getColumnSpec(c);
                DataType cType = cSpec.getType();
                if (cType == null) {
                    status.addError("Column type for column with index '" + c + "' is not set.");
                } else {
                    if (!DataType.class.isAssignableFrom(cType.getClass())) {
                        status.addError("The type of the column with index '" + c + "' is not derived from DataType.");
                    }
                }
                ...
                if (cName == null) {
                    status.addError("Column name for column with index '" + c + "' is not set.");
                } else {
                    // make sure it's unique
                    Integer sameCol = colNames.put(cName, c);
                    if (sameCol != null) {
                        status.addError("Column with index " + c + " has the same name as column " + sameCol + " ('"
                            + cName + "').");
                    }
                }
                ...
                if (values == null) {
                    status.addInfo("No possible values set for column with" + " index '" + c + "'.");
                } else {
                    if (values.size() == 0) {
                        status.addWarning("The container for the possible " + " values of the column with index '" + c
                            + "' is empty!");
                    }
                    // if set they must be not null
                    for (DataCell v : values) {
                        if (v == null) {
                            status.addError("One of the possible values set" + " for the column with index '" + c
                                + "' is" + " null.");
    ...
    /**
     * Returns a string summary for this table which is the entire table content. Note, this call might be time
     * consuming since this method iterates over the table to retrieve all data.
     *
     * {@inheritDoc}
     */
    @Override
    public String toString() {
        ...
        for (int i = 0; i < m_tableSpec.getNumColumns(); i++) {
            if (m_tableSpec.getColumnSpec(i).getType().equals(StringCell.TYPE)) {
                ...
                result.append(
                    sprintDataCell(m_tableSpec.getColumnSpec(i).getName().toString() + "[UNKNOWN!!]", colLength));
            }
        }
        result.append("\n");
