...
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonReaderTransformationParameters.ColumnSpecSettings;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonReaderTransformationParameters.TableSpecSettings;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonReaderTransformationParameters.TableSpecSettingsRef;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonReaderTransformationParameters.TableSpecs;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonReaderTransformationParameters.TransformationElementSettings;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonReaderTransformationParameters.TransformationElementSettings.ColumnNameRef;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonReaderTransformationParameters.TransformationElementSettingsRef;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonTableReaderNodeParameters.FileSelectionRef;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonTableReaderNodeParameters.HowToCombineColumnsOption;
import org.knime.base.node.io.filehandling.csv.reader2.common.CommonTableReaderNodeParameters.HowToCombineColumnsOptionRef;
import org.knime.base.node.io.filehandling.csv.reader2.common.ReaderSpecific.ExternalDataTypeSerializer;
import org.knime.base.node.io.filehandling.csv.reader2.common.ReaderSpecific.ProductionPathProviderAndTypeHierarchy;
...
public final class CommonReaderTransformationParametersStateProviders {

    static final NodeLogger LOGGER = NodeLogger.getLogger(CommonReaderTransformationParametersStateProviders.class);
    ...
    public abstract static class TypedReaderTableSpecsProvider<//
            C extends ReaderSpecificConfig<C>, T, S extends AbstractMultiTableReadConfig<C, DefaultTableReadConfig<C>, T, S>>
          implements StateProvider<Map<FSLocation, TypedReaderTableSpec<T>>>, ReaderSpecific.ConfigAndReader<C, T,S> {
        ...
        @Override
        public final Map<FSLocation, TypedReaderTableSpec<T>> computeState(final NodeParametersInput context) throws StateComputationFailureException {
        ...
        protected abstract void applyParametersToConfig(S config);

        private Map<FSLocation, TypedReaderTableSpec<T>> computeStateFromPaths(final List<FSPath> paths)
            throws StateComputationFailureException {
            final var config = createMultiTableReadConfig();
            applyParametersToConfig(config);
            try {
                final var path = new CommonTableReaderPath();
            ...
            for (var path : paths) {
                try {
                    specs.put(path.toFSLocation(),
                        MultiTableUtils.assignNamesIfMissing(tableReader.readSpec(path, config.getTableReadConfig(), exec)));
    ...
    public abstract static class TableSpecSettingsProvider<T>
        extends DependsOnTypedReaderTableSpecProvider<TableSpecs, T> {

        @Override
        public TableSpecs computeState(final NodeParametersInput context) {

            final var suppliedSpecs = m_specSupplier.get();

            if (suppliedSpecs == null) {
                return new TableSpecs();
            }

            return new TableSpecs(
                suppliedSpecs.entrySet().stream()
                    .map(e -> new TableSpecSettings(e.getKey(), e.getValue().stream()
    ...
    public abstract static class TransformationElementSettingsProvider<T>
        ...
        private Supplier<TableSpecs> m_existingSpecs;
        ...
        public TransformationElementSettings[] computeState(final NodeParametersInput context) {
            ...
            return toTransformationElements(suppliedSpecs == null ? Map.of() : suppliedSpecs, m_howToCombineColumnsSup.get(),
        ...
        private TypedReaderTableSpec<T> getExistingSpecsUnion() {
            final var existingSpecs = toSpecMap(this, m_existingSpecs.get().m_specs);
    ...
    public abstract static class TypeChoicesProvider<T> implements StringChoicesProvider,
        ...
        public void init(final StateProviderInitializer initializer) {
            ...
            initializer.computeOnValueChange(CommonReaderTransformationParameters.TableSpecSettingsRef.class);
            m_specSupplier = initializer.computeFromProvidedState(getTypedReaderTableSpecsProvider());
        }
     ...
     * implementation of {@link CommonReaderTransformationParameters}. This ensures, that the common fields within these
     ...
     * multiple state providers and for the {@link CommonReaderTransformationParametersPersistor}. Instead, extend these
    ...
    private CommonReaderTransformationParametersStateProviders() {
        // Utility class
    }
}
