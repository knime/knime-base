 ...
 */
final class QuotePanel extends JPanel {
    ...
    QuotePanel(final FileReaderNodeSettings settings) {
        ...
        m_frSettings = new FileReaderNodeSettings(settings.getDataFileLocation());
    ...
    private JPanel getQuoteEditPanel() {
        if (m_quoteEditPanel == null) {
            ...
            m_quoteEditPanel.setLayout(new BoxLayout(m_quoteEditPanel, BoxLayout.X_AXIS));
    ...
    private JPanel getTextPanel() {
        if (m_textPanel == null) {
            ...
            textBox.add(new JLabel("Quotes can be multi-character " + "patterns (for example: <quote> )."));
            textBox.add(new JLabel("Escape character (if checked) is always" + " the backslash ('\\')."));
    ...
    private JPanel getQCtrlPanel() {
        if (m_qCtrlPanel == null) {
            ...
            m_qCtrlPanel.setLayout(new BoxLayout(m_qCtrlPanel, BoxLayout.Y_AXIS));
    ...
    private JPanel getQEditPanel() {
        if (m_qEditPanel == null) {
            ...
            m_qEditPanel.setBorder(
                BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "Enter a new quote character:"));
            m_qEditPanel.setLayout(new BoxLayout(m_qEditPanel, BoxLayout.Y_AXIS));
    ...
    private JPanel getQErrorPanel() {
        if (m_qErrorPanel == null) {
            ...
            m_qErrorPanel.setLayout(new BoxLayout(m_qErrorPanel, BoxLayout.X_AXIS));
    ...
    /**
     * Currently, defined quotes are displayed surrounded by spaces, and the esc character added in parantheses. (For
     * example: double quotes will be displayed like ' " (esc:'\')' (without the surounding ticks)).
     *
     ...
     */
    private static String getQuotePattern(final String listEntry) {
        return listEntry.trim().split(" ")[0];
    }

    private static int getEscCharacter(final String listEntry) {
    ...
    /**
     * Creates the string to display in the JList for current quotes. Must work together with the above method that
     * extracts the quote pattern from the string.
     *
     ...
     */
    private static String getListEntry(final Quote quote) {
    ...
    private JPanel getQListPanel() {
        if (m_qListPanel == null) {
            ...
            m_qListPanel.setLayout(new BoxLayout(m_qListPanel, BoxLayout.Y_AXIS));
    ...
    private JPanel getQEditLabelPanel() {
        if (m_qEditLabelPanel == null) {
            ...
            m_qEditLabelPanel.setLayout(new BoxLayout(m_qEditLabelPanel, BoxLayout.X_AXIS));
    ...
    /**
     * Deletes all quotes defined in the passed object, reads the currently listed quotes from the JList and adds them
     * to the settings object.
     *
     * @param settings the settings object to replace the quotes in with the quotes currently defined in the panel
    ...
    /**
     * Transfers the values from the specified object into the components of the panel.
     *
