...
final class CSVTransformationSettingsPersistor extends NodeSettingsPersistorWithConfigKey<CSVTransformationSettings> {
    ...
    public void save(final CSVTransformationSettings transformationSettings, final NodeSettingsWO settings) {
        ...
        final var tableTransformation = new DefaultTableTransformation<Class<?>>(rawSpec, transformations.getFirst(),
            transformationSettings.m_takeColumnsFrom.toColumnFilterMode(), transformations.getSecond(), true, false);
    ...
    private static ImmutableUnknownColumnsTransformation determineUnknownTransformation(
        ...
        if (unknownTransformation.m_type != TypeChoicesProvider.DEFAULT_COLUMNTYPE_ID) {
            forcedUnknownType = PRODUCTION_PATH_PROVIDER.getAvailableDataTypes().stream()
                ...
                .orElseGet(() -> {
                    LOGGER.error(
                        String.format("Unknown and new columns can't be converted to the configured data type."));
                    return null;
                });
        }
        final var unknownColumnsTransformation =
            new ImmutableUnknownColumnsTransformation(unknownColumnsTransformationPosition,
                unknownTransformation.m_includeInOutput, forcedUnknownType != null, forcedUnknownType);
        return unknownColumnsTransformation;
    }
