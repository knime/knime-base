 ...
 */
final class FileReaderPreviewTable implements DataTable {
    private static final NodeLogger LOGGER = NodeLogger.getLogger(FileReaderPreviewTable.class);
    ...
    private final LinkedList<WeakReference<FileReaderPreviewRowIterator>> m_iterators;
     ...
     * @see FileTable
     */
    FileReaderPreviewTable(final DataTableSpec tableSpec, final FileReaderNodeSettings settings,
        final ExecutionContext exec) {
        m_table = new FileTable(tableSpec, settings, exec);
        m_listeners = new CopyOnWriteArrayList<ChangeListener>();
        m_iterators = new LinkedList<WeakReference<FileReaderPreviewRowIterator>>();
        m_errorMsg = null;
        m_errorDetail = null;
        m_errorLine = -1;
    }
    ...
    public RowIterator iterator() {
        FileReaderPreviewRowIterator i = new FileReaderPreviewRowIterator(m_table.iterator(), this);
        synchronized (m_iterators) {
    ...
    /**
     * @return the line number where the error occurred - if an error occurred and an error line number was set.
