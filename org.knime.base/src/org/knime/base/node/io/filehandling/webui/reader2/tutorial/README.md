...
8. Decide whether backwards-compatibility to the previous factory should be satisfied. If not so: Register the node in `plugin.xml` and deprecate the old node:
...
9. If it SHOULD be backwards-compatible: 
     - Write a migration and annotate your NodeParameters class with `@Migration` 
     ...
     - Make the factory extend BackwardsCompatibleWebUITableReaderNodeFactory instead of WebUITableReaderNodeFactory and implement the additional method by taking the implementation from the previous `getPathSettings` method of the old factory 1:1.
