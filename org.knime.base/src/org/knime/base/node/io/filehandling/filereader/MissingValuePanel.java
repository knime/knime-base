...
final class MissingValuePanel extends JPanel {
    ...
    private static Container getTextBox() {
        ...
        result.add(new JLabel("The entered pattern is " + "used in string columns only."));
        result.add(Box.createVerticalStrut(20));
        result.add(new JLabel("If the specified pattern is read as data item" + " from the"));
        result.add(new JLabel("file it will be represented by a data cell" + " with a missing value "));
        result.add(new JLabel("in the output table."));
        result.add(Box.createVerticalStrut(20));
        result.add(new JLabel("This global pattern is overridden by a " + "pattern"));
        result.add(new JLabel("specified for a specific column (when " + "clicking on the "));
    ...
    private void updateWarnBox() {
        ...
        if (!p.isEmpty() && p.trim().isEmpty()) {
            ...
            m_warnLabel.setText("INFO: pattern has spaces at the beginning" + " and/or at the end");
        }
    }
