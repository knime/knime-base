...
public abstract class TransformationParameters<T>
    implements NodeParameters, TransformationParametersCommon.TransformationsMapper<T> {
    ...
    /**
     * Note that this field remains {@code null} until it is updated by the state provider when specs could be computed.
    ...
    public <C extends ReaderSpecificConfig<C>> void saveToConfig(final MultiTableReadConfig<C, T> config,
        ...
        final var columnFilterMode = multiFileReaderParameters.m_howToCombineColumns.toColumnFilterMode();
        ...
        final var tableTransformation = new DefaultTableTransformation<T>(rawSpec, transformations.getFirst(),
            columnFilterMode, transformations.getSecond(), m_enforceTypes, skipEmptyColumns);
    ...
    public TableSpecSettingsWithFsLocation[] getSpecs() {
        return m_specs;
    }
