...
public final class CSVTableReaderNodeSettings implements DefaultNodeSettings {
    ...
    static class AdvancedSettings implements WidgetGroup, PersistableSettings {
        ...
        enum HowToCombineColumnsOption {
                @Label(value = "Fail if different", description = """
                        The node will fail if multiple files are read and not all files have the same table structure.
                        """)
                FAIL(ColumnFilterMode.UNION),

                @Label(value = "Union", description = """
                        Any column that is part of any input file is considered. If a file is missing a column, it
                        is filled up with missing values.
                        """)
