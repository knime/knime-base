 ...
 */
final class FileReaderNodeDialog extends NodeDialogPane {

    private static final NodeLogger LOGGER = NodeLogger.getLogger(FileReaderNodeDialog.class);
    ...
    private static final Delimiter[] DEFAULT_DELIMS = new Delimiter[]{
        ...
        new Delimiter("<none>", false, false, false), new Delimiter(",", false, false, false),
        new Delimiter(" ", true, false, false), new Delimiter("\t", false, false, false),
    ...
    FileReaderNodeDialog(final FileReaderNodeSettings settings) {
        m_frSettings = settings;
        final FlowVariableModel writeFvm = createFlowVariableModel(
            settings.getDataFileLocation().getKeysForFSLocation(), FSLocationVariableType.INSTANCE);
        m_fileChooser = new DialogComponentReaderFileChooser(settings.getDataFileLocation(),
            FileReaderNodeModel.FILEREADER_HISTORY_ID, writeFvm);
    ...
    private JPanel createFileNamePanel() {
        ...
        m_fileChooser.setToolTipText("Enter an URL of an ASCII datafile, select from recent files, or browse");

        fileBox.add(Box.createHorizontalGlue());
        fileBox.add(m_fileChooser.getComponentPanel());
        ...
        m_fileChooser.getSettingsModel().addChangeListener(new ChangeListener() {
     ...
     */
    void rowHeadersSettingsChanged() {
        ...
        if (m_frSettings.getFileHasRowHeaders() && !m_hasRowHeaders.isSelected()) {
            ...
            m_frSettings.setNumberOfColumns(m_frSettings.getNumberOfColumns() + 1);
            // we must create a new colProperty for it - if not already created
            if (m_firstColProp == null) {
                DataColumnSpec firstColSpec = new DataColumnSpecCreator("Col0", StringCell.TYPE).createSpec();
            ...
            analyzeDataFileAndUpdatePreview(true);
        } else if (!m_frSettings.getFileHasRowHeaders() && m_hasRowHeaders.isSelected()) {
     ...
     */
    void colHeadersSettingsChanged() {
        ...
        if (o != DEFAULT_DELIMS[0]) {
            ...
            if (o instanceof Delimiter) {
                ...
                try {
                    ...
                    m_frSettings.addDelimiterPattern(delimStr, selDelim.combineConsecutiveDelims(),
                        selDelim.returnAsToken(), selDelim.includeInToken());
    ...
    /**
     * Loads the settings from the global settings object into the delimiter box and creates the basicDelim vector.
     */
    ...
    /**
     * Pops open the dialog of the columnProperties object of the specified column. This will allow the user to enter
     * new column name, type and missing value. Also changes the domain and 'read from file' flag.
     *
     * @param colIdx the index of the column to get new user settings for
     */
    void setNewUserSettingsForColumn(final int colIdx) {
        ...
        Vector<ColProperty> newColProps = ColPropertyDialog.openUserDialog(f, colIdx, cProps);
    ...
    private void loadSettingsFromInternal(final NodeSettingsRO settings, final PortObjectSpec[] specs) {
        ...
        try {
            ...
            m_frSettings = new FileReaderNodeSettings(settings, m_fileChooser.getSettingsModel());
        } catch (InvalidSettingsException ice) {
            m_frSettings = new FileReaderNodeSettings(m_fileChooser.getSettingsModel());
        }
        ...
        if ((m_frSettings.getColumnProperties() != null) && !m_frSettings.getColumnProperties().isEmpty()) {
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) throws InvalidSettingsException {
        ...
        if (m_frSettings.getNumberOfColumns() < 0) {
            synchronized (m_analysisRunning) {
                ...
                if (!m_analysisRunning.booleanValue()) {
                    if (m_frSettings.getDataFileLocation().getPath().isBlank()) {
                        throw new InvalidSettingsException("No file specified");
                    }
                    // the analysis thread should override the error label
                    setErrorLabelText(
                        "Waiting for file analysis to finish..." + "Click \"Quick Scan\" to cut it short.");
                    analyzeAction();
                }
            }
        }
        ...
        synchronized (m_analysisRunning) {
            if (m_analysisRunning.booleanValue()) {
                throw new InvalidSettingsException("File analysis currently running. Please wait for it to"
                    + " finish, check the settings, and " + "click OK or Apply again");
            } else {
        ...
        if ((errLabel != null) && (errLabel.trim().length() > 0)) {
            throw new InvalidSettingsException("With the current settings" + " an error occurs: " + errLabel);
        }
        if (m_previewTable != null && m_previewTable.getErrorOccurred()) {
            throw new InvalidSettingsException(
                "With the current settings" + " an error occurs when reading the file (line "
                    + m_previewTable.getErrorLine() + "): " + m_previewTable.getErrorMsg());
        }
    ...
    /**
     * Updates the preview table, if a new and valid URL was specified in the data file name text field or the force
     * parameter is set true. It overrides all current settings with the settings from the file analyzer - except when
     * the URL didn't changed and the user has explicitly set some values. For big files, it just shows a button to
     * trigger analysis. The analysis runs in the background, and if it finishes it shows the new content of the file.
     * <p>
     * NOTE: May change the global settings object completely.
     *
     * @param forceAnalyze forces the analysis of the datafile even though it might be the one set in the global
     *            settings (and thus already being analyzed).
     *
     */
    void analyzeDataFileAndUpdatePreview(final boolean forceAnalyze) {

        if (forceAnalyze && !m_frSettings.getDataFileLocation().getPath().isBlank()) {
    ...
    /**
     * If an analysis is currently running it will interrupt it (which causes any analysis results to be discarded). If
     * no analysis is running this method does nothing. The method returns immediately (and doesn't wait for the
     * analysis to finish). It does try to aquire the "m_analysisRunning" lock!
     */
    ...
    /**
     * If no analysis is running it triggers one and waits until its done, otherwise it just waits for the running
     * analysis to finish.
     */
    void waitForAnalyzeAction() {
        synchronized (m_analysisRunning) {
            if (!m_analysisRunning.booleanValue()) {
                analyzeAction();
            }
    ...
    void analyzeAction() {
        ...
        synchronized (m_analysisRunning) {
            ...
            while (m_analysisRunning.booleanValue()) {
                LOGGER.error(
                    "Internal error: Re-entering analysis thread - " + "canceling it - waiting for it to finish...");
                m_analysisExecMonitor.setExecuteInterrupted();
                // wait until it finishes
                try {
                    m_analysisRunning.wait();
    ...
    /**
     * Triggers analysis and returns. After analysis is done, settings are loaded and the preview is shown.
     */
        ...
        ThreadUtils.threadWithContext(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    FileReaderNodeSettings newSettings = FileAnalyzer.analyze(userSettings, m_analysisExecMonitor);
                    ...
                    if (m_analysisExecMonitor.wasCanceled()) {
                         ...
                         */
                        if (newSettings != null) {
                            setAnalWarningText("WARNING: suggested settings " + "are based on a partial file analysis "
                                + "only! Please verify.");
                        }
                    } else {
                        setAnalWarningText("");
                    ...
                    if ((newSettings != null) && !m_analysisExecMonitor.wasInterrupted()) {
     ...
     */
    private FileReaderNodeSettings createPreviewSettings(final FileReaderNodeSettings settings) {
    ...
    private void resetSettings() {
        FileReaderNodeSettings newSettings = new FileReaderNodeSettings(m_fileChooser.getSettingsModel());
        m_frSettings = newSettings;
        m_firstColProp = null;
    ...
    private void recreateColNames(final boolean uniquifyFirstColName) {
        ...
        for (int c = 0; c < colProps.size(); c++) {
            ColProperty cProp = colProps.get(c);
            if ((cProp != null) && (cProp.getUserSettings()) && (!((c == 0) && uniquifyFirstColName))) {
        ...
        for (int c = 0; c < colProps.size(); c++) {
            ...
            if ((cProp.getUserSettings()) && (!((c == 0) && uniquifyFirstColName))) {
    ...
    void advancedSettings() {
        ...
        FileReaderAdvancedDialog advDlg = new FileReaderAdvancedDialog(f, m_frSettings);

        advDlg.setVisible(true);
        // will not continue until user closes the dialog

        if (advDlg.closedViaOk()) {
