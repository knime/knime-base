 ...
 */
class FileReaderSettings extends TokenizerSettings {

    /** The node logger for this class. */
    private static final NodeLogger LOGGER = NodeLogger.getLogger(FileReaderSettings.class);

    /* the list of settings that are stored in here. */

    private SettingsModelReaderFileChooser m_fileChooser;
    ...
    FileReaderSettings(final SettingsModelReaderFileChooser fileChooser) {
     ...
     */
    FileReaderSettings(final FileReaderSettings clonee) {
        super(clonee);
        m_fileChooser = clonee.getDataFileLocation();
     ...
     *
     * @param cfg the config object containing all settings this object will be initialized with
     * @param portsConfig the ports configuration
     * @param fsIdentifier the file system identifier
     * @throws InvalidSettingsException if the passed config object contains invalid or insufficient settings
     */
    FileReaderSettings(final NodeSettingsRO cfg, final SettingsModelReaderFileChooser fileChooser)
        ...
        if (cfg != null) {
            try {
                ...
                throw new InvalidSettingsException(
                    "Illegal config object for " + "file reader settings! Key '" + CFGKEY_HASCOL + "' missing!", ice);
            }
            ...
            try {
                ...
                throw new InvalidSettingsException(
                    "Illegal config object for " + "file reader settings! Key '" + CFGKEY_HASROW + "' missing!", ice);
            }
            ...
            try {
                ...
                throw new InvalidSettingsException(
                    "Illegal config object for " + "file reader settings! Key '" + CFGKEY_IGNOREEMPTY + "' missing!",
                    ice);
            }
            ...
            if (cfg.containsKey(CFGKEY_ROWPREF)) {
                try {
                    ...
                    throw new InvalidSettingsException("Illegal config object for file"
                        + " reader settings! Wrong type of key '" + CFGKEY_HASROW + "'!", ice);
                }
            }
            ...
            if (cfg.containsKey(CFGKEY_MISSINGS)) {
                ...
                try {
                    ...
                    throw new InvalidSettingsException("Illegal config object for file "
                        + "reader settings! Wrong type of key '" + CFGKEY_MISSINGS + "'!", ice);
                }
            ...
            if (cfg.containsKey(CFGKEY_FORMAT)) {
                ...
                try {
                    ...
                    throw new InvalidSettingsException("Illegal config object for file "
                        + "reader settings! Wrong type of key '" + CFGKEY_FORMAT + "'!", ice);
                }
                readFormatParametersFromConfig(formatConf);
            }

            m_globalMissPatternStrCols = cfg.getString(CFGKEY_GLOBALMISSPATTERN, null);
            NodeSettingsRO rowDelimConf = null;
            try {
                rowDelimConf = cfg.getNodeSettings(CFGKEY_ROWDELIMS);
            } catch (InvalidSettingsException ice) {
                throw new InvalidSettingsException("Illegal config object for file reader settings!"
                    + " Not existing or wrong type of key '" + CFGKEY_ROWDELIMS + "'!", ice);
            ...
            if (m_decimalSeparator == m_thousandsSeparator) {
                throw new InvalidSettingsException(
                    "Decimal separator and " + "thousands separator can't be the same character");
            }

            // ignore empty tokens at end of row?
            // It'S optional and default to false, for backward compatibility.
            m_ignoreEmptyTokensAtEOR = cfg.getBoolean(CFGKEY_IGNOREATEOR, false);
            ...
            m_columnNumberDeterminingLine = cfg.getInt(CFGKEY_COLDETERMLINENUM, -1);
    ...
    /**
     * Saves all settings into a {@link org.knime.core.node.NodeSettingsWO} object. Using the cfg object to construct a
     * new FileReaderSettings object should lead to an object identical to this.
     *
    ...
    public void saveToConfiguration(final NodeSettingsWO cfg) {
        if (cfg == null) {
            throw new NullPointerException("Can't save 'file " + "reader settings' to null config!");
        }

        m_fileChooser.saveSettingsTo(cfg);
    ...
    private void readMissingPatternsFromConfig(final NodeSettingsRO missPattConf) {
        if (missPattConf == null) {
            throw new NullPointerException("Can't read missing patterns from null config object");
        }
        int m = 0;
        for (String key : missPattConf.keySet()) {
            // they should all start with "MissPattern"...
            if (key.indexOf(CFGKEY_MISSING) != 0) {
                LOGGER.warn("Illegal missing pattern " + "configuration '" + key + "'. Ignoring it!");
                continue;
            }
            try {
                ...
                LOGGER.warn("Illegal missing pattern " + "configuration '" + key + "' (should be of type"
                    + " string). Ignoring it!");
                continue;
            }
            m++;
    ...
    private void saveMissingPatternsToConfig(final NodeSettingsWO cfg) {
        if (cfg == null) {
            throw new NullPointerException("Can't save missing patterns to null config object");
        }
    ...
    private void readRowDelimitersFromConfig(final NodeSettingsRO rowDelims) throws InvalidSettingsException {
        ...
        for (int rowDelIdx = 0; rowDelims.containsKey(CFGKEY_ROWDELIM + rowDelIdx); rowDelIdx++) {
            ...
            try {
                ...
                LOGGER.warn("Invalid configuration for" + " row delimiter '" + CFGKEY_ROWDELIM + rowDelIdx
    ...
    private void saveRowDelimitersToConfig(final NodeSettingsWO cfg) {
        if (cfg == null) {
            throw new NullPointerException("Can't save 'row delimiters' " + "to null config!");
        }
        int rowDelIdx = 0;
        for (String rowDelim : m_rowDelimiters) {
            ...
            Delimiter delim = getDelimiterPattern(rowDelim);
            if (delim == null) {
                LOGGER.error("Row delimiter '" + rowDelim + "' was not defined with the tokenizer.");
                LOGGER.error("Storing the property 'skip empty lines' " + "with 'false'.");
     ...
     *
     * @param dataFileLocation the location of the data file these settings are for
     */
    public void setDataFileLocationAndUpdateTableName(final String dataFileLocation) {
        m_fileChooser.setPath(dataFileLocation);
    }

    /**
     * @return the location of the file these settings are meant for
     */
    public SettingsModelReaderFileChooser getDataFileLocation() {
        return m_fileChooser;
    }

    /**
     * Set the new character set name that will be used the next time a new input reader is created (see
     * {@link #createNewInputReader()}).
     *
     * @param name any character set supported by Java, or null to use the VM default char set.
     * @throws IllegalArgumentException if the specified name is not supported.
     * @throws java.nio.charset.IllegalCharsetNameException if the specified name is not supported.
     */
    public void setCharsetName(final String name) {
        if (name == null) {
            ...
            if (!Charset.isSupported(name)) {
                throw new IllegalArgumentException("Unsupported charset name '" + name + "'.");
            }
            m_charsetName = name;
        }
    }
    ...
    /**
     * @return a new reader to read from the data file location. It will create a buffered reader, and for zipped
     *         sources a GZIP one. If the data location is not set an exception will fly.
     ...
     */
    public BufferedPathReader createNewInputReader() {
        return new BufferedPathReader(m_fileChooser);
    }

    /**
     * @return the currently set name of the table created by this node. Valid names are not <code>null</code>, but the
     *         method could return null, if no name was set yet.
     */
    public String getTableName() {
        //FIXME: getPureFileName is poorly implemented ... always assumes /
        return getPureFileNameWithExtension(getDataFileLocation().getPath());
    }

    /**
     * @param loc the location to extract the filename from.
     * @return the filename part of the URL without path. Or <code>null</code> if the URL is <code>null</code>.
     */
    private static String getPureFileNameWithExtension(final String loc) {
        if (loc != null) {
            int firstIdx = loc.lastIndexOf('/') + 1;
            if (firstIdx == loc.length()) {
                // last character is '/' ?!?! Filename is empty. Weird anyway.
                return "";
            }
            return loc.substring(firstIdx);
        }
        return "";
    }
     ...
     *
     * @param flag if <code>true</code> the first line in the file will not be considered data, but either ignored or
     *            used as column headers, depending on the column headers set (or not) in this object.
     */
    ...
    /**
     * @return a flag telling if the first line in the file will not be considered data, but either ignored or used as
     *         column headers, depending on the column headers set (or not) in this object.
     */
     ...
     *
     * @param flag if <code>true</code> the first item in each line in the file will not be considered data, but either
     *            ignored or used as row header, depending on the row header prefix set (or not) in this object.
     */
    ...
    /**
     * @return a flag telling if the first item in each line in the file will not be considered data, but either ignored
     *         or used as row header, depending on the row header prefix set (or not) in this object.
     */
    ...
    /**
     * Set a string that will be used as a prefix for each row header. The header generated will have the row number
     * added to the prefix. This prefix - if set - will be used, regardless of any row header read from the file - if
     * there is any.
     *
     * @param rowPrefix the string that will be used to construct the header for each row. The actual row header will
     *            have the row number added. Specify <code>null</code> to clear the prefix.
     */
    ...
    /**
     * @return the string that will be used to construct the header for each row. The actual row header will have the
     *         row number added. If this returns <code>null</code>, the row header from the file will be used - if any,
     *         otherwise the {@link #DEF_ROWPREFIX}.
     */
    ...
    /**
     * Will add a delimiter pattern that will terminate a row. Row delimiters are always token (=column) delimiters. Row
     * delimiters will always be returned as separate token by the filereader. You can define a row delimiter that was
     * previously defined a token delimiter. But only, if the delimiter was not set to be included in the token.
     * Otherwise you will get a {@link IllegalArgumentException}.
     *
     * @param rowDelimPattern the row delimiter pattern. Row delimiters will always be token delimiters and will always
     *            be returned as separate token.
     * @param skipEmptyRows if set <code>true</code>, multiple consecutive row delimiters will be combined and returned
     *            as one
     */
    public void addRowDelimiter(final String rowDelimPattern, final boolean skipEmptyRows) {
        ...
        if (existingDelim != null) {
            if (existingDelim.includeInToken()) {
                ...
                throw new IllegalArgumentException("Can't define a row " + "delimiter ('" + rowDelimPattern
                    + "') that was defined as token delimiter before" + " that should be included in the tokens");
            }
        }
    ...
    /**
     * Removes the row delimiter with the specified pattern. Even though the above method changes an existing column
     * delimiter to being a row delim, this function completely deletes the row delimiter (instead of being aware that
     * it might have been a col delim before and changing it back to a col delim).
     *
     * @param pattern the row delimiter to delete must not be null. <code>null</code> is always a row delimiter.
     * @return a delimiter object specifying the deleted delimiter, or <code>null</code> if no row delimiter with the
     *         pattern existed
     */
    public Delimiter removeRowDelimiter(final String pattern) {
        if (pattern == null) {
            throw new NullPointerException("Can't remove <null> as row delimiter.");
        }
    ...
    /**
     * Blows away all defined row delimiters! After a call to this function no row delimiter will be defined (except
     * <code>null</code>).
     */
     ...
     * @return <code>true</code> if the pattern is a row delimiter. <code>null</code> is always a row delimiter.
    ...
    public boolean isRowDelimiter(final String pattern) {
        ...
        assert (!m_rowDelimiters.contains(pattern)) || (getDelimiterPattern(pattern) != null);
    ...
    /**
     * @param ignoreEm pass <code>true</code> to have the file reader not return empty lines from the data file
     */
    ...
    /**
     * Returns <code>true</code> if the file reader combines multiple consecutive row delimiters with this pattern (i.e.
     * it skips empty rows if it finds multiple if these (and only these) row delimiters). The method throws an
     * {@link IllegalArgumentException} at you if the specified pattern is not a row delimiter.
     *
    ...
    public boolean combinesMultipleRowDelimiters(final String pattern) {
        if (!m_rowDelimiters.contains(pattern)) {
            throw new IllegalArgumentException("The specified pattern '" + pattern + "' is not a row delimiter.");
        }
    ...
    /**
     * Specifies a pattern that, if read in for the specified column, will be considered placeholder for a missing
     * value, and the data table will contain a missing cell instead of that value then.
     *
     * @param colIdx the index of the column this missing value is set for
     * @param pattern the pattern specifying the missing value in the data file for the specified column. Can be
     *            <code>null</code> to delete a previously set pattern.
     */
    ...
    /**
     * Returns the pattern that, if read in for the specified column, will be considered placeholder for a missing
     * value, and the data table will contain a missing cell instead of that value then.
     *
     * @param colIdx the index of the column the missing value is asked for
     * @return the pattern that will be considered placeholder for a missing value in the specified column. Or
     *         <code>null</code> if no patern is set for that column.
     */
    ...
    /**
     * @return the character that is considered decimal separator in the data (token) for a double type column
     */
     ...
     *
     * @param sep the new decimal character to set for doubles. Can't be the same character as the thousands separator.
     */
    public void setDecimalSeparator(final char sep) {
        if (sep == m_thousandsSeparator) {
            throw new IllegalArgumentException(
                "Can't set the decimal " + "separator to the same character as the thousands " + "separator.");
    ...
    /**
     * @param thousandsSeparator the thousandsSeparator to set. If set to '\0' it will not be applied. Can't be the same
     *            as the decimal separator.
     */
    public void setThousandsSeparator(final char thousandsSeparator) {
        if ((thousandsSeparator != '\0') && (thousandsSeparator == m_decimalSeparator)) {
            throw new IllegalArgumentException(
                "Can't set the thousands " + "separator to the same character as the decimal " + "separator.");
    ...
    /**
     * @return <code>true</code> if additional empty tokens should be ignored at the end of a row (if they are not
     *         needed to build the row)
     */
     ...
     * Sets this flag.
     *
     * @param ignoreThem if <code>true</code>, additional empty tokens will be ignored at the end of a row (if they are
     *            not needed to build the row)
     */
    ...
    /**
     * @param supportShortLines if set true lines with too few data elements will be accepted and filled with missing
     *            values.
     */
    ...
    /**
     * @return true, if lines with too few data items are accepted (they will be filled with missing values, if read),
     *         or false, it the reader fails when it comes across a short line (the default).
     */
    ...
    /**
     * Sets a new pattern which is translated into a missing value if read from the data file in a string column. Is is
     * used only for columns that don't have their own missing value pattern set (and that are of type string).
     *
     * @param pattern the new pattern to recognize missing values in string columns. Set to <code>null</code> to clear
     *            it.
     */
    ...
    /**
     * Returns the pattern that, if read in, will be translated into a missing value (in string columns only). It is
     * overridden by the column specific missing value pattern. If it is not defined, null is returned.
     *
     * @return the pattern for missing values, for all string columns. Or null if not defined.
     */
    ...
    /**
     * @return the line number in the file that determined the number of columns. Or -1 if not set yet (or no file
     *         analysis took place).
     */
     ...
     *
     * @param lineNumber the line number in the file that determined the number of columns.
     */
    ...
    /**
     * @return the maximum number of lines that should be read from the source. If -1 is returned all rows should be
     *         read.
     */
     ...
     *
     * @param maxNum the new maximum. If set to -1 all rows of the source will be read, otherwise no more than the
     *            specified number.
     */
    ...
    /**
     * Method to check consistency and completeness of the current settings. It will return a {@link SettingsStatus}
     * object which contains info, warning and error messages. Or if the settings are alright it will return null.
     *
     * @param openDataFile tells whether or not this method should try to access the data file. This will - if set
     *            <code>true</code> - verify the accessibility of the data.
     * @param tableSpec the spec of the DataTable these settings are for. If set <code>null</code> only a few checks
     *            will be performed - the ones that are possible without the knowledge of the structure of the table
     * @return a SettingsStatus object containing info, warning and error messages, or <code>null</code> if no messages
     *         were generated (i.e. all settings are just fine).
     */
     ...
     * @param openDataFile specifies if we should check the accessability of the data file
     * @param tableSpec the spec of the DataTable these settings are for. If set <code>null</code> only a few checks
     *            will be performed - the ones that are possible without the knowledge of the structure of the table
     */
    protected void addStatusOfSettings(final SettingsStatus status, final boolean openDataFile,
        final DataTableSpec tableSpec) {

        // check the data file location. It's required.
        if (m_fileChooser.getPath().isBlank()) {
        ...
        for (String rowDelim : m_rowDelimiters) {
            Delimiter delim = getDelimiterPattern(rowDelim);
            if (delim == null) {
                status.addError("Row delimiter '" + rowDelim + " is not defined" + "being a token delimiter.");
                continue;
            }
            if (delim.includeInToken()) {
                status.addError("Row delimiter '" + rowDelim + "' is set to" + " be included in the token.");
            }
            if (!delim.returnAsToken()) {
                status.addError("Row delimiter '" + rowDelim + "' is not set to" + " be returned as separate token.");
            }
        }
        if (m_rowDelimiters.size() == 0) {
            status.addWarning("No row delimiters are defined! The table will"
                + " be read into one row (and supernumerous cells will" + " be ignored).");
        }
        ...
        if (tableSpec != null) {
            ...
            if (numCols > m_missingPatterns.size()) {
                status.addInfo("Not all columns have patterns for missing" + "values assigned.");
            } else if (numCols < m_missingPatterns.size()) {
                status.addError("There are more patterns for missing values" + " defined than columns in the table.");
            } else {
                for (Iterator<String> pIter = m_missingPatterns.iterator(); pIter.hasNext();) {
                    if (pIter.next() == null) {
                        status.addInfo("Not all columns have patterns for " + "missing values assigned.");
                    }
    ...
    private void openDataFile(final SettingsStatus status) {
        try (final BufferedPathReader reader = createNewInputReader();
    ...
    public String toString() {
        ...
        if (m_fileChooser.getPath().isBlank()) {
            ...
            res.append(m_fileChooser.getPath());
        }
        res.append("'\n");
        ...
        res.append("Ignore empty tokens at the end of row: " + m_ignoreEmptyTokensAtEOR + "\n");
