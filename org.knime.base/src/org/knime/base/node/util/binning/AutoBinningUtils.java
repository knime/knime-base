...
public class AutoBinningUtils {
    ...
    public static class AutoBinner {
        ...
        public PortObject[] createNodeOutput(final BufferedDataTable inData, final ExecutionContext exec)
            ...
            var table = exec.createColumnRearrangeTable(inData, createRearranger(translator, inSpec), exec);
        ...
        private PMMLPreprocDiscretizeTranslator
            ...
            var config = new Configuration(m_settings.columnNames(), createBins(edgesMap, m_settings.binNaming()));
    ...
    static IntPair findStreakEndpoints(final List<Double> values, final int beginning, final boolean ceil) {
        interface DoubleBiPredicate {
            boolean test(double a, double b); // NOSONAR seriously, Sonar? This does not need a javadoc...
        }

        DoubleBiPredicate maybeCeilEquals = ceil //
    ...
    static List<BinBoundary> createEdgesForEqualCount( //
        ...
        // the next bin boundary may never be less than this index
        var minimumIndexOfNextBinBoundary = 1;

        // naive guess for the next bin boundary index, based on the expected number of values per bin.
        var guessForNextBinBoundaryIndex = countPerBin;

        for (int i = 1; i < edges.length - 1; i++) {
            exec.checkCanceled();

            if (guessForNextBinBoundaryIndex < tableValues.size()) {
                var streakEndpoints = findStreakEndpoints(tableValues, guessForNextBinBoundaryIndex, integerBounds);
                var potentialBoundaryLeft = streakEndpoints.first();

                // note: since the endpoints are inclusive and our upper bin boundaries are open, we
                // don't actually want to put the boundary where the right streak ends, but rather
                // one index later than that.
                var potentialBoundaryRight = Math.min(1 + streakEndpoints.second(), tableValues.size() - 1);

                // how far are we shifting the index to the left or right to avoid splitting
                // identical values across bins, compared to the 'naive' bin position guess?
                var leftDiff = (potentialBoundaryLeft - minimumIndexOfNextBinBoundary);
                var rightDiff = (potentialBoundaryRight - minimumIndexOfNextBinBoundary);

                // we pick the endpoint that results in the smallest difference from the expected
                // bin position (but make sure that we don't go below the startIndex, i.e. the
                // position of the previously established bin edge).
                var actualBinBoundaryIndex =
                    (potentialBoundaryLeft >= minimumIndexOfNextBinBoundary && leftDiff <= rightDiff) //
                        ? potentialBoundaryLeft //
                        : potentialBoundaryRight;

                edges[i] = maybeCeil.applyAsDouble(tableValues.get(actualBinBoundaryIndex));

                minimumIndexOfNextBinBoundary = actualBinBoundaryIndex + 1;
                guessForNextBinBoundaryIndex = actualBinBoundaryIndex + countPerBin;
            } else {
                edges[i] = edges[i - 1];
            }
        }
        ...
        try (var it = table.iterator()) {
            for (int rowIndex = 0; it.hasNext(); ++rowIndex) {
                ...
                for (int col : colIndices) {
                    ...
                    if (minValuesByColIdx.getOrDefault(col, Double.POSITIVE_INFINITY) > val) {
                        minValuesByColIdx.put(col, val);
                    }
                    if (maxValuesByColIdx.getOrDefault(col, Double.NEGATIVE_INFINITY) < val) {
                        minValuesByColIdx.put(col, val);
                    }
                }
            }
     ...
     */
    static Map<String, List<Configuration.Bin>> createBins(final Map<String, List<BinBoundary>> edgesMap,
