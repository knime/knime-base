...
class SampleDataNodeParameters implements NodeParameters {
    ...
    @Widget(title = "Cluster count",
        description = "Comma-separated list defining the number of clusters to generate in each universe "
            + "(e.g., \"2, 3\" creates 2 clusters in the first universe and 3 in the second).")
    @TextInputWidget
    String m_clusterCount = "2, 2";

    @Persistor(UniverseSizesPersistor.class)
    @Widget(title = "Universe sizes",
        description = "Comma-separated list specifying the number of attributes (dimensions) for each universe "
            + "(e.g., \"2, 3\" means first universe has 2 attributes, second has 3).")
    @TextInputWidget
    ...
    @Widget(title = "Noise fraction",
        description = "Proportion of randomly distributed data points that don't belong to any cluster. "
            + "Value between 0 (no noise) and 1 (all noise).")
    ...
    @Widget(title = "Random seed",
        description = "Fixed value to ensure reproducible data generation. "
