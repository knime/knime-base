...
public final class BufferedFileReader extends BufferedReader {
    ...
    public static BufferedFileReader createNewReader(final URL dataLocation, final String charsetName,
        ...
        return createReader(dataLocation.toString(), () -> openStream(dataLocation, timeout), charsetName,
            getFileSize(dataLocation));
    }

    private static long getFileSize(final URL dataLocation) {
        long fileSize = 0;
        try {
            final File f = FileUtil.getFileFromURL(dataLocation);
            if (f.exists()) {
                fileSize = f.length();
            }
    ...
    /**
     * Creates a new reader reading from the supplied input stream using the provided character set. The returned reader
     * can be asked for the number of bytes read from the stream ({@link #getNumberOfBytesRead()}).<br>
     * If the specified file is compressed, it will try to create a ZIP stream (and the byte counts refer both to the
     * compressed file).<br>
     * In addition this reader can be asked for the current line it is reading from ({@link #getCurrentLine()})and the
     * current line number ({@link #getCurrentLineNumber()}).
     ...
     * @param charsetName the character set to use. Must be supported by the VM
     * @param streamSupplier the supplier to open the stream to read from
     * @param fileSize the size of the file to be read
     * @return reader reading from the stream.
     *
     * @throws IOException if something went wrong when opening the stream.
     * @throws java.nio.charset.IllegalCharsetNameException If the given charset name is illegal
     * @throws java.nio.charset.UnsupportedCharsetException If no support for the named charset is available in this
     ...
     * @since 4.4
     */
    public static BufferedFileReader createReader(final String fileName,
        final CheckedExceptionSupplier<InputStream, IOException> streamSupplier, final String charsetName,
        ...
        try {
            ...
            ByteCountingStream sourceStream = new ByteCountingStream(new BufferedInputStream(streamSupplier.get()));
            ...
            try {
                ...
                sourceStream = new ByteCountingStream(new BufferedInputStream(streamSupplier.get()));
                try {
            ...
            throw new IOException("Can't access '" + fileName + "'. (" + msg + ")", e);
        }
    }

    private static InputStream openStream(final URL dataLocation, final Integer timeout) throws IOException {
        return timeout != null ? FileUtil.openStreamWithTimeout(dataLocation, timeout)
