...
final class TimerinfoNodeModel extends WebUINodeModel<TimerinfoNodeSettings> implements InactiveBranchConsumer {
    ...
    private static void reportThisLayer(final WorkflowManager wfm, final BufferedDataContainer result,
        ...
        for (NodeContainer nc : wfm.getNodeContainers()) {
            if (((nc instanceof WorkflowManager) || (nc instanceof SubNodeContainer)) && (depth > 0)) {
                WorkflowManager nestedWorkflowManager;
                // Metanode
                if (nc instanceof WorkflowManager workflowManager) {
                    nestedWorkflowManager = workflowManager;
                    reportThisLayer(nestedWorkflowManager, result, depth-1, toplevelprefix,
                        componentResolution, includeComponentIO);
                } else { // Component
                    SubNodeContainer component = (SubNodeContainer)nc;
                    nestedWorkflowManager = component.getWorkflowManager();
                    applyComponentResolutionPolicyOnThisLayer(nestedWorkflowManager, nc,
                        componentResolution, includeComponentIO, result, depth, toplevelprefix);
                }
            } else { // Node
                if (includeComponentIO) {
                    result.addRowToTable(createTimerInfoTableRow(nc, toplevelprefix));
                } else {
                    filterComponentInputAndOutputNodes(nc, result, toplevelprefix);
                }
            }
        }
    }
    ...
    private static void applyComponentResolutionPolicyOnThisLayer(final WorkflowManager nestedWorkflowManager,
        ...
        if (!nestedWorkflowManager.isUnlocked()) {
            result.addRowToTable(createTimerInfoTableRow(nc, toplevelprefix));
        } else if (componentResolution == ComponentResolutionPolicy.COMPONENTS) {
            result.addRowToTable(createTimerInfoTableRow(nc, toplevelprefix));
        } else if (componentResolution == ComponentResolutionPolicy.NODES) {
            reportThisLayer(nestedWorkflowManager, result, depth-1, toplevelprefix,
                componentResolution, includeComponentIO);
        } else {
            result.addRowToTable(createTimerInfoTableRow(nc, toplevelprefix));
    ...
    private static void filterComponentInputAndOutputNodes(final NodeContainer nc,
        final BufferedDataContainer result, final NodeID toplevelprefix) {
        var nodeClassName = nc instanceof NativeNodeContainer nativeNodeContainer
                ? nativeNodeContainer.getNodeModel().getClass() : "n/a";
        if (!nodeClassName.equals(VirtualSubNodeInputNodeModel.class) &&
            !nodeClassName.equals(VirtualSubNodeOutputNodeModel.class)) {
            result.addRowToTable(createTimerInfoTableRow(nc, toplevelprefix));
        }
    }
