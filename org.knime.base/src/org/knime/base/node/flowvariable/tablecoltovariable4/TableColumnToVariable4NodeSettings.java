...
public class TableColumnToVariable4NodeSettings implements DefaultNodeSettings {
    ...
    TableColumnToVariable4NodeSettings(final DefaultNodeSettingsContext context) {
        final var spec = context.getPortObjectSpec(0);
        if (spec.isPresent() && spec.get() instanceof DataTableSpec dataSpec) {
            this.m_column = dataSpec.stream()//
    ...
    @Widget(title = "If value in cell is missing", description = "Handling of missing values")
    @ValueSwitchWidget
    ...
    static final class MissingOperationMigration implements NodeSettingsMigration<MissingOperation> {
        ...
        public static MissingOperation load(final NodeSettingsRO settings) throws InvalidSettingsException {
