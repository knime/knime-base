...
public class VariableToTable4NodeModel2 extends WebUINodeModel<VariableToTable4NodeSettings> {

    protected VariableToTable4NodeModel2(final WebUINodeConfiguration configuration) {
    ...
    protected PortObjectSpec[] configure(final PortObjectSpec[] inSpecs,
        ...
        try (final var converter = new VariableToDataColumnConverter()) {
            final var selectedCols = List.of(
                modelSettings.m_filter.getSelected(variables.stream().map(FlowVariable::getName).toArray(String[]::new),
                    createTableSpec(variables, converter)));
            final var selectedVariables =
                variables.stream().filter(variable -> selectedCols.contains(variable.getName())).toList();
    ...
    protected PortObject[] execute(final PortObject[] inObjects, final ExecutionContext exec,
        ...
        try (final var converter = new VariableToDataColumnConverter()) {
            ...
            cont.addRowToTable(createTableRow(exec, converter, modelSettings.m_rowName, selectedVariables));
            cont.close();
            return new BufferedDataTable[]{cont.getTable()};
        }
    }

    final static private DataRow createTableRow(final ExecutionContext exec, final VariableToDataColumnConverter conv,
        final String rowId, final Collection<FlowVariable> variables) {
    ...
    final static private DataTableSpec createTableSpec(final Collection<FlowVariable> variables,
