...
import org.knime.base.node.viz.format.number.NumberFormatManagerNodeSettings.NegativeNumberFormat;
...
final class NumberFormatManagerNodeModel extends WebUINodeModel<NumberFormatManagerNodeSettings> {
     ...
     */
    @Override
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs, final NumberFormatManagerNodeSettings modelSettings)
        throws InvalidSettingsException {
        var valueFormatHandler = handlerFor(modelSettings);
        var inSpec = inSpecs[0];
        String[] numberColumnNames = numberColumns(inSpec);
        var result = createOutputSpec(inSpec, modelSettings.m_columnsToFormat.getSelected(numberColumnNames, inSpec), valueFormatHandler);
    ...
    protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec,
        ...
        final var outputTableSpec =
            createOutputSpec(inSpec, modelSettings.m_columnsToFormat.getSelected(numberColumnNames, inSpec), valueFormatHandler);
     ...
     * @return
     */
    private String[] numberColumns(final DataTableSpec inSpec) {
     ...
     * @throws InvalidSettingsException
     */
    private static ValueFormatHandler handlerFor(final NumberFormatManagerNodeSettings settings) throws InvalidSettingsException {
        var formatter = new NumberFormatter.Builder()//
                ...
                .build();
        return new ValueFormatHandler(new ValueFormatModelNumber(formatter, settings.m_negativeNumberFormat == NegativeNumberFormat.RED_COLOR));
    }
