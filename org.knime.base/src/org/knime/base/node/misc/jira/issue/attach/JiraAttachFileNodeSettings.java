...
// Replaced file widget with simple text input path to avoid dependency on file widget module
import org.knime.node.parameters.widget.text.TextInputWidget;
import org.knime.base.node.misc.jira.conn.JiraConnectionPortObjectSpec;

@SuppressWarnings("restriction")
public final class JiraAttachFileNodeSettings implements NodeParameters {
    @Section(title = "Jira Server") interface ServerSection {}
    @Section(title = "Attachment") @After(ServerSection.class) interface AttachSection {}
    ...
    @Widget(title = "Issue Key", description = "Issue key, e.g. PROJ-123") @TextInputWidget(placeholder = "PROJ-123") @Layout(AttachSection.class) @Persist(configKey = "issue/key") public String m_key = "";

    @Widget(title = "File Path", description = "Absolute path to the file to upload") @TextInputWidget(placeholder = "/path/to/file") @Layout(AttachSection.class) @Persist(configKey = "attach/filePath") public String m_filePath = "";
}
