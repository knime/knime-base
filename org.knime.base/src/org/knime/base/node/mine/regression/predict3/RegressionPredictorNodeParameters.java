...
class RegressionPredictorNodeParameters implements NodeParameters {
    ...
    interface HasCustomPredictionNameRef extends ParameterReference<Boolean> {
    }

    @Widget(title = "Use custom prediction column",
            description = "Allows you to specify a customized name for the prediction column "
                ...
                + "is used as default.")
    @ValueReference(HasCustomPredictionNameRef.class)
    @Persist(configKey = RegressionPredictorSettings.CFG_HAS_CUSTOM_PREDICTION_NAME)
    boolean m_hasCustomPredictionName; // false

    static final class HasCustomPredictionNameIsTrue implements EffectPredicateProvider {
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getBoolean(HasCustomPredictionNameRef.class).isTrue();
        }
    }

    @Widget(title = "Prediction column name", description = "The name for the prediction column.")
    @TextInputWidget
    @Persist(configKey = RegressionPredictorSettings.CFG_CUSTOM_PREDICTION_NAME)
    @Effect(type = EffectType.SHOW, predicate = HasCustomPredictionNameIsTrue.class)
    String m_customPredictionName = "";
}
