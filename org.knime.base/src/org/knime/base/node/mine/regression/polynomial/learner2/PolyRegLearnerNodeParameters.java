...
final class PolyRegLearnerNodeParameters implements NodeParameters {
    ...
    @Migration(IncludedColumnsMigration.class)
    ...
    @Widget(title = "Missing values in input data", description = """
            ...
            """)
    @ValueSwitchWidget
    @Migration(MissingValueHandlingMigration.class)
    MissingValueMode m_missingValueHandling = MissingValueMode.FAIL;

    static final class MissingValueHandlingMigration implements DefaultProvider<MissingValueMode> {

        @Override
        public MissingValueMode getDefault() {
            return MissingValueMode.IGNORE;
        }
    ...
    static final class MissingValueHandlingPersistor implements NodeParametersPersistor<MissingValueMode> {
        ...
        public MissingValueMode load(final NodeSettingsRO settings) throws InvalidSettingsException {
            var legacyValue = settings.getString(PolyRegLearnerSettings.CFG_MISSING_VALUE_HANDLING,
    ...
    static final class IncludedColumnsMigration implements NodeParametersMigration<ColumnFilter> {
        static ColumnFilter loadColumnFilter(final NodeSettingsRO settings) {
            final var included = settings.getStringArray(PolyRegLearnerSettings.CFG_SELECTED_COLUMNS, new String[0]);
            final var includeAll = settings.getBoolean(PolyRegLearnerSettings.CFG_INCLUDE_ALL, false);
            return new ColumnFilterBuilder().withMode(ColumnFilterBuilder.Mode.MANUAL).withManuallySelected(included)
                .withIncludeUnknownColumns(includeAll).build();
        }

        @Override
        public List<ConfigMigration<ColumnFilter>> getConfigMigrations() {
            return List.of(ConfigMigration.builder(IncludedColumnsMigration::loadColumnFilter)
                .withDeprecatedConfigPath(PolyRegLearnerSettings.CFG_SELECTED_COLUMNS)
                .withDeprecatedConfigPath(PolyRegLearnerSettings.CFG_INCLUDE_ALL)
                .withMatcher(settings -> settings.containsKey(PolyRegLearnerSettings.CFG_SELECTED_COLUMNS)).build());
        }
    }
