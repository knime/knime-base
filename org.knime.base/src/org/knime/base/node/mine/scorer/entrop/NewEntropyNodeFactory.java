...
public class NewEntropyNodeFactory extends NodeFactory<EntropyNodeModel>
     ...
     */
    @Override
    public NodeDialogPane createNodeDialogPane() { // wrap modern dialog to support legacy flow variables UI
    ...
    static final String FULL_DESCRIPTION = "Scorer for clustering results given a reference clustering. Connect the"
        + " table containing the reference clustering to the first input port (the table should contain a column with"
        + " the cluster IDs) and the table with the clustering results to the second input port (it should also contain"
        + " a column with some cluster IDs). Select the respective columns in both tables from the dialog. After"
        + " successful execution, the view will show entropy values (the smaller the better) and some quality value"
        + " (in [0,1] - with 1 being the best possible value, as used in <a "
        + "href=\"http://www.uni-konstanz.de/bioml/bioml2/publications/Papers2007/WiBe07_fcum_ijar/Fuzzy%20Clustering%20in%20Parallel%20Universes_submitted.pdf" // NOSONAR
        + "\">Fuzzy Clustering in Parallel Universes</a>, section 6: \"Experimental results\").";

    static final String VIEW_DESCRIPTION = "Simple statistics on the clustering such as number of clusters being found,"
    ...
    protected NodeDescription createNodeDescription() throws SAXException, IOException, XmlException {
        return WebUINodeFactory.createNodeDescription( //
            ...
            "entropyscore.png", //
            new PortDescription[]{ //
                createPortDescription("Reference clustering", "Table containing reference clustering."),
                createPortDescription("Clustering to score", "Table containing clustering (to score).")},
            new PortDescription[]{ //
                createPortDescription("Quality Table",
            ...
            FULL_DESCRIPTION, //
            EntropyScorerNodeSettings.class, //
            null, // view settings
            VIEW_DESCRIPTION, //
            NodeType.Other, //
            new String[0]);
    ...
    @SuppressWarnings("deprecation")
    private static final PortDescription createPortDescription(final String name, final String description) {
        return new PortDescription(name, BufferedDataTable.TYPE, description);
    }
}
