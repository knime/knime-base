...
final class UngroupNodeParameters implements NodeParameters {
    ...
    UngroupNodeParameters(final NodeParametersInput context) {
        ...
        if (spec.isPresent()) {
            final var collectionColumns =
                ColumnSelectionUtil.getCompatibleColumns(spec.get(), CollectionDataValue.class);
     ...
     */
    static final class CollectionColumnsProvider extends CompatibleColumnsProvider {
        CollectionColumnsProvider() {
            super(CollectionDataValue.class);
        }
    }

    /**
     * Persistor that stores/loads the column filter under the legacy key "columnNames" to maintain compatibility with
     * the existing UngroupNodeModel.
     */
