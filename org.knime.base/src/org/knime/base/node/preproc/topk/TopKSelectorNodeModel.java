...
final class TopKSelectorNodeModel extends WebUINodeModel<TopKSelectorNodeSettings> {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs, final TopKSelectorNodeSettings modelSettings)
        ...
        final var sk = Arrays
            .stream(modelSettings.m_sortingCriteria).map(crit -> new SortKeyItem(crit.m_column.m_selected,
                crit.m_sortingOrder == SortingOrder.ASCENDING, crit.m_stringComparison == StringComparison.NATURAL))
            .toList();
    ...
    protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec,
        ...
        final var sortKey = Arrays.stream(modelSettings.m_sortingCriteria)
            .map(crit -> new SortKeyItem(crit.m_column.m_selected, crit.m_sortingOrder == SortingOrder.ASCENDING,
                Arrays.stream(crit.m_column.m_compatibleTypes)
                    .anyMatch(type -> StringValue.class.getName().equals(type))
                    && crit.m_stringComparison == StringComparison.NATURAL))
            .toList();

        final BufferedDataTable table = inData[0];
        if (table.size() < modelSettings.m_k) {
    ...
    private static TopKSelector createElementSelector(final Comparator<DataRow> comparator,
        ...
        if (modelSettings.m_k == 1 && topKMode == TopKMode.TOP_K_ROWS) {
            return new TopSelector(inverted);
        } else if (topKMode == TopKMode.TOP_K_ALL_ROWS_W_UNIQUE) {
            return new HeapTopKUniqueRowsSelector(inverted, modelSettings.m_k);
        } else {
            return new HeapTopKSelector(inverted, modelSettings.m_k);
        }
    }
