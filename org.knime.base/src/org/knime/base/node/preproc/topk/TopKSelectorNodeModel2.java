...
public class TopKSelectorNodeModel2 extends WebUINodeModel<TopKSelectorNodeSettings> {

    protected TopKSelectorNodeModel2(final WebUINodeConfiguration configuration) {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs, final TopKSelectorNodeSettings modelSettings)
        ...
        final var sk = Arrays.stream(modelSettings.m_sortingCriteria).map(crit -> new SortKeyItem(crit.getColumn(),
            crit.getSortingOrder() == SortingOrder.ASCENDING, crit.getStringComparison() == StringComparison.NATURAL))
            .toList();
        final DataTableSpec dts = inSpecs[0];
        final List<String> missing = SortKeyItem.getMissing(sk, dts, TopKSelectorNodeModel2::isRowKey);
    ...
    protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec,
        ...
        CheckUtils.checkSetting(modelSettings.m_sortingCriteria != null && modelSettings.m_sortingCriteria.length != 0,
            "No columns have been specified to select the top rows of. Set in the node configuration.");
        final var sortKey = Arrays.stream(modelSettings.m_sortingCriteria).map(crit -> new SortKeyItem(crit.getColumn(),
            crit.getSortingOrder() == SortingOrder.ASCENDING, crit.getStringComparison() == StringComparison.NATURAL))
            .toList();
        ...
        final boolean missingsLast = modelSettings.m_missingToEnd;
        final var dts = table.getDataTableSpec();
        final var rc = SortKeyItem.toRowComparator(dts, sortKey, missingsLast, TopKSelectorNodeModel2::isRowKey);
    ...
    private static TopKSelector createElementSelector(final Comparator<DataRow> comparator,
        ...
        if (modelSettings.m_amount == 1 && topKMode == TopKMode.TOP_K_ROWS) {
            ...
            return new HeapTopKUniqueRowsSelector(inverted, modelSettings.m_amount);
        } else {
            return new HeapTopKSelector(inverted, modelSettings.m_amount);
        }
    }
