...
public abstract class AbstractSamplingWebUINodeModel<T extends DefaultNodeSettings> extends WebUINodeModel<T> {
    ...
    protected void validateSettings(final T settings) throws InvalidSettingsException {
        ...
        switch (countMode) {
            case RELATIVE:
                validatePercentage(getPercentage(settings));
                break;
            case ABSOLUTE:
                validateCount(getCount(settings));
                break;
            default:
                throw new InvalidSettingsException("Unknown method: " + countMode);
        }

        if (SamplingMode.STRATIFIED == getSamplingMode(settings) && (getClassColumn(settings) == null)) {
    ...
    private static void validateCount(final int count) throws InvalidSettingsException {
        if (count < 0) {
    ...
    protected IRowFilter getSamplingRowFilter(final BufferedDataTable in, final ExecutionMonitor exec, final T settings)
        ...
        final var countMode = getCountMode(settings);
        int rowCount = CountMode.RELATIVE == countMode ? //
            (int)((getPercentage(settings) / 100.0) * in.size()) : //
            getCount(settings);
        ...
        if (SamplingMode.RANDOM == samplingMode || SamplingMode.STRATIFIED == samplingMode) {
            ...
            if (SamplingMode.RANDOM == samplingMode) {
                ...
                return new StratifiedSamplingRowFilter(in, getClassColumn(settings), rowCount, rand, exec);
            }
        }
