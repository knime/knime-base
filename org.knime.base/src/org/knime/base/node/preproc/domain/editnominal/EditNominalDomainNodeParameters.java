...
public class EditNominalDomainNodeParameters implements NodeParameters {
    ...
    static class ColumnDomainParameters implements NodeParameters {
        ...
        static final class InsertNewAddedDomainValues implements StateProvider<String[]> {
            ...
            public String[] computeState(final NodeParametersInput parametersInput)
                throws StateComputationFailureException {
                String[] newValues = m_newValues.get();
                String[] domainValues = m_domainValues.get();
                String[] sortedValues = m_sortedValues.get();
    ...
    static final class ColumnDomainParametersPersistor
        ...
        public List<ColumnDomainParameters> load(final NodeSettingsRO settings) throws InvalidSettingsException {
            ...
            for (var columnSettingIter = columns.iterator(); columnSettingIter.hasNext();) {
                ...
                var columnName = columnSettingIter.next();
        ...
        public void save(final List<ColumnDomainParameters> param, final NodeSettingsWO settings) {
            ...
            for (final var columnDomain : param) {
                ...
                final var createdDomainValuesCells = new DataCell[columnDomain.m_newDomainValues.length];
                for (int i = 0; i < columnDomain.m_newDomainValues.length; i++) {
                    createdDomainValuesCells[i] = new StringCell(columnDomain.m_newDomainValues[i]);
                }
                columnSettings.addDataCellArray(CREATED_DOMAIN_VALUES_KEY, createdDomainValuesCells);
                final var columnDomainValueOrderingCells = new DataCell[columnDomain.m_sortedDomainValues.length];
                for (int i = 0; i < columnDomain.m_sortedDomainValues.length; i++) {
                    columnDomainValueOrderingCells[i] = new StringCell(columnDomain.m_sortedDomainValues[i]);
                }
                columnSettings.addDataCellArray(COLUMN_DOMAIN_VALUE_ORDERING_KEY, columnDomainValueOrderingCells);
            }
        }
