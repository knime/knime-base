...
public final class ColumnHeaderInsertSettings implements DefaultNodeSettings {
    ...
    ColumnHeaderInsertSettings(final DataTableSpec dictSpec) {
        ...
        m_valueColumn = findDefaultValueColumn(dictSpec).map(DataColumnSpec::getName).orElse(null);
    }

    private static Optional<DataColumnSpec> findDefaultValueColumn(final DataTableSpec dictSpec) {
        return findFirstWithSuitableType(dictSpec, t -> t.equals(StringCell.TYPE))//
            .or(() -> findFirstWithSuitableType(dictSpec, t -> t.isCompatible(StringValue.class)));
    }

    private static Optional<DataColumnSpec> findFirstWithSuitableType(final DataTableSpec dictSpec,
        final Predicate<DataType> typePredicate) {
        return dictSpec.stream()//
            .filter(c -> typePredicate.test(c.getType()))//
            .findFirst();
    }
}
