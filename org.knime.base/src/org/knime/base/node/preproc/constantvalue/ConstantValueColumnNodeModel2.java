...
final class ConstantValueColumnNodeModel2
    ...
    protected void validateSettings(final ConstantValueColumnNodeSettings settings) throws InvalidSettingsException {
        ...
        var firstUsedEmptyAppendedColumnName = Arrays.stream(settings.m_newColumnSettings) //
            .filter(s -> s.m_replaceOrAppend == AppendOrReplace.APPEND && s.m_columnNameToAppend.isEmpty()) //
            .findFirst();
        if (firstUsedEmptyAppendedColumnName.isPresent()) {
        ...
        var outputColumnNames = Arrays.stream(settings.m_newColumnSettings) //
            .map(s -> s.m_replaceOrAppend == AppendOrReplace.APPEND ? s.m_columnNameToAppend : s.m_columnNameToReplace) //
            .toList();
        Predicate<String> nameIsInColumnListMultipleTimes = name -> outputColumnNames.stream() //
            .filter(name::equals) //
            .count() > 1;
        var firstDuplicateColumnName = outputColumnNames.stream() //
            .distinct() //
            .filter(nameIsInColumnListMultipleTimes) //
            .findFirst();
        if (firstDuplicateColumnName.isPresent()) {
            throw new InvalidSettingsException(
                "The column name '" + firstDuplicateColumnName.get() + "' is used multiple times.");
        }
    }
    ...
    protected ColumnRearranger createColumnRearranger(final DataTableSpec spec,
        ...
        if (modelSettings.m_newColumnSettings.length == 0) {
            setWarningMessage("No columns to add or replace.");
        }
