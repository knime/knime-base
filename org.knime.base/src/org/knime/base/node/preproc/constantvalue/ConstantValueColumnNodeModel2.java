...
import org.knime.base.node.preproc.constantvalue.ConstantValueColumnNodeSettings.NewColumnSettings.ReplaceOrAppend;
...
final class ConstantValueColumnNodeModel2
    ...
    protected void validateSettings(final ConstantValueColumnNodeSettings settings) throws InvalidSettingsException {
        for (var newColumnSettings : settings.m_newColumnSettings) {
            if (!newColumnSettings.m_type.isValidData(newColumnSettings.m_value)) {
                throw new InvalidSettingsException("The value '" + newColumnSettings.m_value
                    + "' is not a valid value for the selected type " + newColumnSettings.m_type + ".");
            }

            if (newColumnSettings.m_replaceOrAppend == ReplaceOrAppend.APPEND
                && newColumnSettings.m_columnNameToAppend.isEmpty()) {
                throw new InvalidSettingsException("The column name to append must not be empty.");
            }
        }
    }
    ...
    protected ColumnRearranger createColumnRearranger(final DataTableSpec spec,
        ...
        for (var newColumnSettings : modelSettings.m_newColumnSettings) {
            var outputColumnName = newColumnSettings.m_replaceOrAppend == ReplaceOrAppend.APPEND //
