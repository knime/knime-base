 ...
 * @author AI Migration Pipeline 42631f4
 */
@LoadDefaultsForAbsentFields
public class TableDifferNodeParameters implements NodeParameters {
    ...
    enum FailureMode {
            ...
            @Label(value = "Different table specs",
                description = "The node will fail if any of the selected columns does not exist in "
                    + "the other table, or the columns differ in type or domain. Note that differing column positions are being ignored.")
            DIFFERENT_SPECS,
    ...
    @Widget(title = "Select columns from reference table", description = """
            ...
            selection (all columns with a certain type are included). In case of manual selection, the
            behavior for new columns (i.e. that are not available at the time you configure the node) can be
            specified as either Enforce exclusion (new columns are excluded and therefore not used for
            comparison) or Enforce inclusion (new columns are included and therefore used for comparison).
            """)
     ...
     */
    public TableDifferNodeParameters() {
     ...
     */
    public TableDifferNodeParameters(final NodeParametersInput context) {
     ...
     */
    public TableDifferNodeParameters(final DataTableSpec spec) {
