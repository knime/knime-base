...
final class CategoryToNumberNodeParameters implements NodeParameters {
    ...
    @Widget(title = "Append columns", description = """
                ...
                replace the columns in the include list.
            """)
    @Persist(configKey = "append_columns")
    boolean m_appendColumns = true;

    @Widget(title = "Column name suffix", description = """
                The column name of the computed columns is the name of the column in the input appended by this suffix.
            """)
    @TextInputWidget
    @Persist(configKey = "column_suffix")
    String m_columnSuffix = " (to number)";
    ...
    @Widget(title = "Map missing cells to...", description = """
     ...
     */
    private static class DataCellOptionalIntPersistor implements NodeParametersPersistor<Optional<Integer>> {
        ...
        public Optional<Integer> load(final NodeSettingsRO settings) throws InvalidSettingsException {
            var cell = settings.getDataCell(m_configKey, DataType.getMissingCell());
            if (cell == null || cell.isMissing()) { // NOSONAR
                return Optional.empty();
            }
            if (cell instanceof IntValue intCell) {
                return Optional.of((intCell).getIntValue());
            }
