...
final class CreateTableStructureNodeModel2 extends WebUINodeModel<CreateTableStructureNodeSettings> {

    protected CreateTableStructureNodeModel2(final WebUINodeConfiguration configuration) {
    ...
    private static DataTableSpec createSpec(final CreateTableStructureNodeSettings modelSettings)
        ...
        if (modelSettings.m_createMode == CreateMode.BASIC) {
            String prefix = modelSettings.m_basicModeSettings.m_columnPrefix;
            if (prefix == null || prefix.isEmpty()) {
                throw new InvalidSettingsException("Invalid (empty) prefix");
            }
            String typeS = modelSettings.m_basicModeSettings.m_colType.name();
            DataType type;
            try {
            ...
            int count = modelSettings.m_basicModeSettings.m_numberOfColumns;
            DataColumnSpec[] colSpecs = new DataColumnSpec[count];
            for (int i = 0; i < count; i++) {
                colSpecs[i] = new DataColumnSpecCreator(prefix + i, type).createSpec();
            }
            return new DataTableSpec("Table Structure", colSpecs);
        } else if (modelSettings.m_createMode == CreateMode.CUSTOM) {
            final long distinct = Arrays.stream(modelSettings.m_customModeSettings).map(setting -> setting.m_columnName)
                .distinct().count();
            if (distinct != modelSettings.m_customModeSettings.length) {
                throw new InvalidSettingsException("Duplicate column name found");
            }
            final var specs = Arrays.stream(modelSettings.m_customModeSettings).map(
                setting -> (new DataColumnSpecCreator(setting.m_columnName, setting.m_colType.getType())).createSpec())
                .toArray(DataColumnSpec[]::new);
            return new DataTableSpec("Table Structure", specs);
        } else {
            throw new InvalidSettingsException("Unknown Create Mode: '" + modelSettings.m_createMode.name() + "'");
        }
    }
}
