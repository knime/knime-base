...
final class CreateTableStructureNodeSettings implements DefaultNodeSettings {
    ...
    ColumnSettings[] m_columnSettings = new ColumnSettings[]{new ColumnSettings()};
    ...
    static final class ColumnSettings implements DefaultNodeSettings {
        ...
        ColumnSettings(final String columnName, final String colType) {
        ...
        ColumnSettings(final String columnName, final DataType colType) {
            this.m_columnName = columnName;
            this.m_colType = colType.getName();
        }
        ...
        @ChoicesWidget(choices = DataTypeChoicesProvider.class)
        String m_colType = StringCellFactory.TYPE.getName();

        DataType getDataType() {
            return DataTypeRegistry.getInstance().availableDataTypes().stream()
                .filter(type -> type.getName().equals(m_colType)).findFirst().orElseThrow();
        }
    }
    ...
    static final class ColumnSettingsMigration implements NodeSettingsMigration<ColumnSettings[]> {
        ...
        static ColumnSettings[] load(final NodeSettingsRO settings) throws InvalidSettingsException {
            final var colCount = settings.getInt(COUNT_KEY, 5);
            ...
            for (int i = 0; i < colCount; i++) {
                columns[i] = new ColumnSettings(colPrefix + (i + 1), createDataTypeFromName(colType));
            }
            return columns;
        }
        ...
        public List<ConfigMigration<ColumnSettings[]>> getConfigMigrations() {
            return List.of(//
                ConfigMigration.builder(ColumnSettingsMigration::load)
                    .withMatcher(settings -> settings.containsKey(COUNT_KEY)) //
                    .withDeprecatedConfigPath(COUNT_KEY)//
