...
public final class LongFilterOperators implements FilterOperators {
    ...
    public List<FilterOperatorFamily<? extends FilterValueParameters>> getOperatorFamilies() {
        return List.of( //
            new LongEqualityFamily(), //
            new LongComparisonFamily(), //
            new IntegralNumberPatternOperatorFamily());
    }

    private static final class LongEqualityFamily extends EqualsOperatorFamily<LongCell, LongParameters> {

        private LongEqualityFamily() {
            super(LongCell.TYPE, LongParameters.class);
        }
            ...
            if (!isCompLongVal && !isCompIntVal) {
                throw FilterValidationUtil
                    .createInvalidSettingsException(builder -> builder.withSummary(FilterValidationUtil
                        ...
                        .addResolutions(
                            // change input
                            FilterValidationUtil.resolutionChangeInput(LongCell.TYPE, IntCell.TYPE),
                            // reconfigure
                            FilterValidationUtil.resolutionChangeInput(colType)));
            }
    ...
    // there is no common value type for IntValue and LongValue, so we need to be a bit more generic than we'd like
    static final class IntegralNumberPatternOperatorFamily
        extends PatternOperatorFamily<DataValue, PatternFilterParameters> {

        IntegralNumberPatternOperatorFamily() {
            super(DataValue.class, PatternFilterParameters.class);
        }

        @Override
        protected Set<Class<? extends DataValue>> supportedTypes() {
            // since we opt out of the default compatibility check by specifying `DataValue` as our concrete value type,
            // we need to override this method to restrict the type further
            return Set.of(IntValue.class, LongValue.class);
        }

        @Override
        protected Predicate<DataValue> createPredicate(final DataColumnSpec runtimeColumnSpec,
            final DataType configureColumnType, final boolean isRegex, final PatternFilterParameters params)
            throws InvalidSettingsException {

            final var runtimeType = runtimeColumnSpec.getType();
            // IntCell implements LongValue, but LongCell does not implement IntValue, so we need to check
            // the smaller type first
            final Function<DataValue, String> toStringFunction;
            if (runtimeType.isCompatible(IntValue.class)) {
            ...
            return PatternFilterUtils.createPredicate(params.m_pattern, isRegex, true, toStringFunction);
        }
