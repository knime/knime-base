...
/**
 * Parameters for integral number pattern filter operators, where case sensitivity is not applicable.
 *
 * Use for Integer, Long, and row number pattern filters.
 *
...
public class PatternFilterParameters implements FilterValueParameters {
    ...
    @Before(Pattern.class)
    interface BeforePattern {
    }
    ...
    @Widget(title = "Pattern",
        description = "The pattern to filter for. The format depends on the operator (regex or wildcard).")
    @Layout(Pattern.class)
    public String m_pattern = "";

    @Override
    public DataValue[] stash() {
        return new DataValue[]{new StringCell(m_pattern)};
