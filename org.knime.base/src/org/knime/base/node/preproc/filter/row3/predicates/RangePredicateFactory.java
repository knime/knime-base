...
import org.knime.base.node.preproc.filter.row3.predicates.RangePredicateFactory.DoubleRangePredicate;
import org.knime.base.node.preproc.filter.row3.predicates.RangePredicateFactory.IntRangePredicate;
import org.knime.base.node.preproc.filter.row3.predicates.RangePredicateFactory.LongRangePredicate;
import org.knime.base.node.preproc.filter.row3.predicates.RowReadPredicateFactory.IntIntComparator;
import org.knime.base.node.preproc.filter.row3.predicates.RowReadPredicateFactory.RangeOperator;
...
abstract class RangePredicateFactory extends AbstractPredicateFactory {
    ...
    private static final class RangeIntPredicateFactory extends RangePredicateFactory {
        ...
        public IndexedRowReadPredicate createPredicate(final int columnIndex, final DynamicValuesInput inputValues)
            ...
            final var predicate = createRowReadPredicate(columnIndex, inputValues);
            return (idx, row) -> predicate.test(row);
        }

        private Predicate<RowRead> createRowReadPredicate(final int columnIndex, final DynamicValuesInput inputValues)
            throws InvalidSettingsException {
            ...
            if (refCell instanceof IntCell intCell) {
                ...
                return new IntIntComparator(m_operator).createPredicate(r -> r.<IntValue> getValue(columnIndex),
                    intCell);
            }
