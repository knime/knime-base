...
final class NominalValueRowSplitterNodeModel extends WebUINodeModel<NominalValueRowSplitterNodeSettings> {
    ...
    private boolean matches(final DataRow row) {
        ...
        if (dc.isMissing()) {
            ...
            ).m_missingValueHandling == NominalValueRowSplitterNodeSettings.MissingValueHandling.INCLUDE;
        } else {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs, // NOSONAR method is old and well-tested. Complexity is OK.
        ...
        if (selectedColumn != null && selectedColumn.length() > 0) {
            ...
            // all values excluded?
            var includeMissing =
                settings.m_missingValueHandling == NominalValueRowSplitterNodeSettings.MissingValueHandling.INCLUDE;
