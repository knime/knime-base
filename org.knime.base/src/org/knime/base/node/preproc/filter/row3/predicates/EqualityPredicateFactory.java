...
abstract class EqualityPredicateFactory extends AbstractPredicateFactory {
    ...
    private static final class EqualityRowNumberPredicateFactory extends RowNumberPredicateFactory {

        private final boolean m_matchEqual;

        private EqualityRowNumberPredicateFactory(final boolean matchEqual) {
            m_matchEqual = matchEqual;
        }

        @Override
        public IndexedRowReadPredicate createPredicate(final DynamicValuesInput inputValues)
            throws InvalidSettingsException {
            final var refCell = getCellAtOrThrow(inputValues, 0);
            long ref;
            if (refCell instanceof IntCell intCell) {
