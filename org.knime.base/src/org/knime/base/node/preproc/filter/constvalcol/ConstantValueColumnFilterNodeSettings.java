...
class ConstantValueColumnFilterNodeSettings implements DefaultNodeSettings {
    ...
    interface FilterModeRef extends Reference<FilterMode> {
    }
    ...
    interface FilterNumericRef extends Reference<Optional<Double>> {
    }
    ...
    interface FilterStringRef extends Reference<Optional<String>> {
    }
    ...
    interface FilterMissingRef extends Reference<Boolean> {
    }
        ...
        public Optional<Message> computeState(final DefaultNodeSettingsContext context) {
            return m_filterNumericSupplier.get().isEmpty() && m_filterStringSupplier.get().isEmpty()
                && m_filterMissingSupplier.get().equals(Boolean.FALSE)
                    ? Optional.of(new Message("At least one partial filtering option should be selected.",
                        ConstantValueColumnFilterNodeFactory.WARNING_NO_FILTER_SELECTED, MessageType.WARNING))
                    : Optional.empty();
        }
    }
