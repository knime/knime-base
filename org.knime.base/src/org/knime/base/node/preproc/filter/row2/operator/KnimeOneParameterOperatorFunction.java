...
/**
 * {@link KnimeOperatorFunction} for operators that have only a single parameter.
 *
 * @author Perla Gjoka, KNIME GmbH, Konstanz, Germany
 */
final class KnimeOneParameterOperatorFunction implements KnimeOperatorFunction {

    private final Function<String, DataCell> m_convertToDataCell;

    private final Function<DataCell, Predicate<DataCell>> m_cellPredicateFunction;
     ...
     * @param cellPredicateFunction creates a predicate DataCell of the just created DataCell from user's input.
     * @param rangeFunction maps user input about the row index into a range of needed rows.
     */
    public KnimeOneParameterOperatorFunction(final Function<String, DataCell> convertToDataCell,
        final Function<DataCell, Predicate<DataCell>> cellPredicateFunction,
    ...
    public RowPredicateFactory apply(final OperatorParameters parameters) {
        ...
        final DataCell parameter = m_convertToDataCell.apply(parameters.getValues()[0]);
        return new DefaultRowPredicateFactory(m_cellPredicateFunction.apply(parameter), parameters.getColumnSpec(),
            m_rangeFunction.apply(parameters.getColumnSpec(), parameters.getValues()));
    }
}
