...
public final class RowNumberParameters implements SingleCellValueParameters<LongCell> {
    ...
    public void applyStash(final DataValue[] stashedValues) {
        if (stashedValues == null || stashedValues.length == 0 || stashedValues[0] == null) {
            return;
        }
        final DataValue value = stashedValues[0];
        if (value instanceof LongCell longCell) {
            m_value = longCell.getLongValue();
        } else if (value instanceof IntCell intCell) {
            m_value = intCell.getIntValue();
        } else if (value instanceof DoubleCell doubleCell) {
            m_value = (long)doubleCell.getDoubleValue();
        } else if (value instanceof StringCell stringCell) {
            try {
                m_value = Long.parseLong(stringCell.getStringValue());
            } catch (final NumberFormatException e) { // NOSONAR best-effort parsing
