...
/**
 * Before 5.5 renaming a column to an other existing one without renaming the other could lead to the other column being
 * renamed as well by appending a (#index) suffix. Since then non-renamed columns are kept as is and only new names are
 * made unique using indices.
 *
 * E.g. for columns "A" and "B" when renaming "A" to "B" before 5.5 the result would be "B" (formerly "A") and "B(#1)"
 * (formerly "B") whereas since 5.5 the result would be "B (#1)" (formerly "A") and "B".
 *
 * @author Paul Bärnreuther
 * @since 5.8
 */
public class ColumnNameReplacerNodeParametersWithLegacyReplacementStrategy1 extends ColumnNameReplacerNodeSettings {

    @Migration(LoadFalseForOldNodes2.class)
    boolean m_resolveNamingCollisionsOnlyForRenamedColumns = true;
    ...
    protected Set<String> getColumnsToCompareAgainstForCollisions(final Set<String> extantColumnNames,
        ...
        if (m_resolveNamingCollisionsOnlyForRenamedColumns) {
    ...
    protected void addToRenamesMap(final Map<String, String> renames, final String oldName,
        ...
        if (m_resolveNamingCollisionsOnlyForRenamedColumns) {
            ...
            if (replacement.wasReplaced()) {
                ...
                // always add to renames map, even if the name did not change
