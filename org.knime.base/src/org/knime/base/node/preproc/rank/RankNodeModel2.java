...
import org.knime.base.node.preproc.rank.RankNodeSettings.SortingCriterionSettings;
...
public class RankNodeModel2 extends WebUINodeModel<RankNodeSettings> {

    RankNodeModel2(final WebUINodeConfiguration configuration) {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs, final RankNodeSettings modelSettings)
        ...
        SortingCriterionSettings[] rankCols = modelSettings.m_sortingCriteria;
        ...
        checkAllContained(Arrays.stream(rankCols).map(SortingCriterionSettings::getColumn).toArray(String[]::new),
            "ranking", tableSpec);
        ...
        if (modelSettings.m_rankColumnName.isEmpty()) {
    ...
    private static DataTableSpec createOutSpec(final DataTableSpec inSpec, final RankNodeSettings modelSettings) {
        return new DataTableSpec(inSpec,
            new DataTableSpec(createRankColSpec(modelSettings.m_rankDataType, modelSettings.m_rankColumnName)));
    }
    ...
    private static BufferedDataTable sortTable(final ExecutionContext exec, final BufferedDataTable table,
        ...
        return new BufferedDataTableSorter(table,
            toList(Arrays.stream(modelSettings.m_sortingCriteria).map(SortingCriterionSettings::getColumn)
                .map(col -> SpecialColumns.ROWID.getId().equals(col)
                    ? BufferedDataTableSorter.ROWKEY_SORT_SPEC.getName() : col) // Allow sorting on RowKey
                .toArray(String[]::new)),
            order, modelSettings.m_missingtoEnd).sort(exec);
    }
    ...
    private static BufferedDataTable appendRank(final ExecutionContext exec, final BufferedDataTable sortedTable,
        final RankNodeSettings modelSettings) throws CanceledExecutionException, InvalidSettingsException {
        ...
        int[] rankColIndices = getIndicesFromColNameList(Arrays.stream(modelSettings.m_sortingCriteria)
            .map(SortingCriterionSettings::getColumn).toArray(String[]::new), spec);
        // append rank column
        columnRearranger.append(
            new RankCellFactory2(createRankColSpec(modelSettings.m_rankDataType, modelSettings.m_rankColumnName),
                groupColIndices, rankColIndices, RankMode.fromString(modelSettings.m_rankMode.name()),
                modelSettings.m_rankDataType == RankDataType.LONG));
