...
final class RankNodeModel extends WebUINodeModel<RankNodeSettings> {
    ...
    private static BufferedDataTable sortTable(final ExecutionContext exec, final BufferedDataTable table,
        ...
        return new BufferedDataTableSorter(table,
            Arrays.stream(modelSettings.m_sortingCriteria).map(RankingCriterionSettings::getColumn)
    ...
    private static BufferedDataTable retainRowOrder(final ExecutionContext exec, final String rowOrderColumn,
        ...
        // recover row order
        out = new BufferedDataTableSorter(out, new ArrayList<String>(Collections.singleton(rowOrderColumn)),
            new boolean[]{true}).sort(exec);
        // remove order column
