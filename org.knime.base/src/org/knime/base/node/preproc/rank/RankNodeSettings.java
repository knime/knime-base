...
public class RankNodeSettings implements DefaultNodeSettings {
    ...
    enum RankMode {
            @Label(value = "Standard", description = """
                    ...
                    """)
            STANDARD(StandardRankAssigner2::new), //
            ...
            @Label(value = "Ordinal", description = """
                    ...
                    unique ranks.
                    """)
            ORDINAL(i -> new OrdinalRankAssigner());

        private final Function<int[], RankAssigner> m_rankAssignerFactory;

        RankMode(final Function<int[], RankAssigner> rankAssignerFactory) {
        ...
        static RankMode fromLegacyRankMode(final String legacyName) throws InvalidSettingsException {
            var legacyEnum = Stream.of(RankNodeModel.RankMode.values())//
                .filter(m -> m.toString().equals(legacyName))//
                .findFirst()//
                .orElseThrow(() -> new InvalidSettingsException(
                    String.format("Unknown RankMode '%s' encountered.", legacyName)));
            return Stream.of(RankMode.values())//
                .filter(m -> m.name().equals(legacyEnum.name()))//
                .findFirst()//
                .orElseThrow(() -> new InvalidSettingsException(
                    String.format("Unknown RankMode '%s' encountered.", legacyEnum.name())));
        }
    }
    ...
    SortingCriterionSettings[] m_sortingCriteria = new SortingCriterionSettings[]{};
    ...
    static final class CategoryColumnSettingsMigration implements NodeSettingsMigration<ColumnFilter> {
        ...
        private static ColumnFilter load(final NodeSettingsRO settings) {
            final var columns = settings.getStringArray(KEY, new String[]{});
        ...
        public List<ConfigMigration<ColumnFilter>> getConfigMigrations() {
            return List.of(//
                ConfigMigration.builder(CategoryColumnSettingsMigration::load) //
                    .withMatcher(settings -> settings.containsKey(KEY)) //)
    ...
    static final class RankModeSettingsMigration implements NodeSettingsMigration<RankMode> {
        ...
        private static RankMode load(final NodeSettingsRO settings) throws InvalidSettingsException {
            final var rankMode = settings.getString(KEY, RankNodeModel.RankMode.STANDARD.toString());
    ...
    static final class LegacySortingSettingsMigration implements NodeSettingsMigration<SortingCriterionSettings[]> {
        ...
        private static SortingCriterionSettings[] load(final NodeSettingsRO settings) {
            final var columns = settings.getStringArray(COL_KEY, new String[0]);
        ...
        public List<ConfigMigration<SortingCriterionSettings[]>> getConfigMigrations() {
            return List.of(//
                ConfigMigration.builder(LegacySortingSettingsMigration::load) //
                    .withMatcher(settings -> settings.containsKey(COL_KEY)) //)
