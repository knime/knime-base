...
final class RankNodeSettings implements DefaultNodeSettings {
    ...
    static final class CategoryColumnChoiceProvider implements ColumnChoicesStateProvider {
        ...
        public DataColumnSpec[] columnChoices(final DefaultNodeSettingsContext context) {
            final var criterion = Arrays.stream(m_criterion.get()).map(RankingCriterionSettings::getColumn).toList();
        ...
        public RankingCriterionSettings computeState(final DefaultNodeSettingsContext context) {
            ...
            final var usedColumns =
                Stream.concat(Arrays.stream(this.m_categoryColumns.get().getSelectedFromFullSpec(spec.get())),
                    Arrays.stream(this.m_criterion.get()).map(RankingCriterionSettings::getColumn)).toList();
            final var firstAvailableColumn =
                spec.get().stream().filter(colSpec -> !usedColumns.contains(colSpec.getName())).findFirst();
            return firstAvailableColumn.map(col -> new RankingCriterionSettings(col))
                .orElse(new RankingCriterionSettings());
        }
     ...
     * @author Martin Sillye, TNG Technology Consulting GmbH
     */
    static final class RankingCriterionSettings implements DefaultNodeSettings {

        RankingCriterionSettings(final String column, final SortingOrder sortingOrder) {
            m_column = column;
        ...
        @ChoicesWidget(choicesProvider = CriterionColumnChoicesProvider.class, showRowKeysColumn = true)
        String m_column;
