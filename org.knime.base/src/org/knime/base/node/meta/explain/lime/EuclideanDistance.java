 ...
 */
enum EuclideanDistance implements Distance {
        INSTANCE;
    ...
    public double calculate(final DoubleIterable a, final DoubleIterable b) {
        final DoubleIterator iteratorA = a.iterator();
        final DoubleIterator iteratorB = b.iterator();
        double squaredDiffs = 0;
        while (iteratorA.hasNext()) {
            CheckUtils.checkArgument(iteratorB.hasNext(), "b has fewer elements than a.");
            final double valA = iteratorA.next();
            final double valB = iteratorB.next();
            final double diff = valA - valB;
            squaredDiffs += diff * diff;
        }
        CheckUtils.checkArgument(!iteratorB.hasNext(), "b has more elements than a.");
