...
class LoopStartIntervalDynamicNodeParameters implements NodeParameters {
    ...
    @NumberInputWidget(minValidationProvider = FromMinimumValidationProvider.class)
    ...
    static final class FromMinimumValidationProvider implements StateProvider<MinValidation> {

        private Supplier<Double> m_fromSupplier;

        private Supplier<Double> m_toSupplier;
        ...
        public void init(final StateProviderInitializer initializer) {
            ...
            m_toSupplier = initializer.computeFromValueSupplier(ToRef.class);
        ...
        public MinValidation computeState(final NodeParametersInput parametersInput)
            ...
            if (m_fromSupplier.get() == null || m_toSupplier.get() == null || m_stepSizeSupplier.get() == null) {
                throw new StateComputationFailureException();
            }
            if ((m_fromSupplier.get() < m_toSupplier.get()) && m_stepSizeSupplier.get() < 0) {
                return new MinValidation() {

                    @Override
                    protected double getMin() {
                        return m_toSupplier.get();
                    }

                    @Override
                    public String getErrorMessage() {
                        return "From must be bigger than to if step size is negative.";
                    }
    ...
    @ValueReference(ToRef.class)
    double m_to = 1.0;

    static final class ToRef implements ParameterReference<Double> {
    }
    ...
    @Widget(title = "Loop variable is", description = """
