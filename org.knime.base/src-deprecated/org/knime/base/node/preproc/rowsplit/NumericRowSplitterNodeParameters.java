...
final class NumericRowSplitterNodeParameters implements NodeParameters {
    ...
    @Widget(title = "Lower bound value", description = """
            ...
            """)
    @ValueReference(LowerBoundValueRef.class)
    @Effect(predicate = IsLowerBoundEnabled.class, type = EffectType.SHOW)
    double m_lowerBoundValue;

    static final class LowerBoundValueRef implements ParameterReference<Double> {
    }
    ...
    @Widget(title = "Upper bound value", description = """
            ...
            the first output port.
            """)
    @NumberInputWidget(minValidationProvider = LowerBoundMaxValueValidationProvider.class)
    ...
    static final class LowerBoundMaxValueValidationProvider implements StateProvider<MinValidation> {

        Supplier<Boolean> m_isLowerBoundDefinedSupplier;

        Supplier<Double> m_lowerBoundValueSupplier;

        @Override
        public void init(final StateProviderInitializer initializer) {
            m_isLowerBoundDefinedSupplier = initializer.computeFromValueSupplier(IsLowerBoundEnabled.class);
            m_lowerBoundValueSupplier = initializer.computeFromValueSupplier(LowerBoundValueRef.class);
        }

        @Override
        public MinValidation computeState(final NodeParametersInput parametersInput)
            throws StateComputationFailureException {
            final var isLowerBoundDefined = m_isLowerBoundDefinedSupplier.get();
            final var lowerBoundValue = m_lowerBoundValueSupplier.get();
            if (lowerBoundValue == null || !isLowerBoundDefined) {
                return null;
            }
