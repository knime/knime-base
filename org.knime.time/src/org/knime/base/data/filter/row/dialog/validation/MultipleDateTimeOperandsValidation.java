...
import org.knime.base.data.filter.row.dialog.DateTimeOperatorValidation;
 ...
 * @since 4.0
 */
public class MultipleDateTimeOperandsValidation implements DateTimeOperatorValidation {
    ...
    public ValidationResult apply(final OperatorParameters parameters) {
        ...
        if (values == null || values.length == 0) {
            result.addError(ValidationResult.COMMON_ERROR_ID, "At least one value is required");
        } else {
            final DataType type = parameters.getColumnSpec().getType();

            for (int i = 0; i < values.length; i++) {
                final int index = i; // lambda function can accept only a final variable

                checkValue(values[index], type, message -> result.addError(index, message));
            }
        }
        return result;
    }
