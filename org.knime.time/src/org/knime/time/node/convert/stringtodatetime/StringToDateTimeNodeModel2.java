    ...
    final class StringToDateTimeCellFactory extends SingleCellFactory {
        ...
        public DataCell getCell(final DataRow row, final long rowIndex) {
            ...
            try {
                ...
                m_warningListener.addRowIssue(0, rowIndex, message);
        ...
        public void afterProcessing() {
            if (m_warningListener.getIssueCount() > 0) {
                m_warningListener
                    .withSummary("%s cell%s failed to parse".formatted(m_warningListener.getIssueCount(),
                        m_warningListener.getIssueCount() == 1 ? "" : "s"))
                    .build().ifPresent(StringToDateTimeNodeModel2.this::setWarning);
            }
        }
    }
}
