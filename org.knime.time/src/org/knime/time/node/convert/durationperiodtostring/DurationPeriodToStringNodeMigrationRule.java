 ...
 */
public class DurationPeriodToStringNodeMigrationRule extends NodeMigrationRule {
    ...
    protected Class<? extends NodeFactory<?>> getReplacementNodeFactoryClass(final MigrationNode migrationNode,
        ...
        return DurationPeriodToStringNodeFactory2.class;
    }

    @Override
    protected MigrationNodeMatchResult match(final MigrationNode migrationNode) {
        if ("org.knime.time.node.convert.durationperiodtostring.DurationPeriodToStringNodeFactory"
    ...
    protected void migrate(final MigrationNode migrationNode, final MigrationNodeMatchResult matchResult)
        ...
        final var oldMigration = migrationNode.getOriginalNodeModelSettings(); //No new model, using old settings
        oldMigration.copyTo(newModelSettings);
        pair.getSecond().copyTo(getNewNodeVariableSettings(migrationNode));
    }

    private static Pair<DurationPeriodToStringNodeSettings, NodeSettingsRO>
        performMigration(final MigrationNode migrationNode) throws MigrationException {
        final var newSettings = new DurationPeriodToStringNodeSettings();
