 ...
 */
public class DurationToNumberNodeMigrationRule extends NodeMigrationRule {
    ...
    protected Class<? extends NodeFactory<?>> getReplacementNodeFactoryClass(final MigrationNode migrationNode,
        ...
        return DurationToNumberNodeFactory2.class;
    }

    @Override
    protected MigrationNodeMatchResult match(final MigrationNode migrationNode) {
        if ("org.knime.time.node.convert.durationtonumber.DurationToNumberNodeFactory"
    ...
    static final String DURATION_FIELD_KEY = "duration_field_select";

    static final String DATE_TYPE_KEY = "date_type_select";

    @SuppressWarnings("restriction")
    private static Pair<DurationToNumberNodeSettings, NodeSettingsRO>
        ...
        final var newSettings = new DurationToNumberNodeSettings();
        try {
            newSettings.m_filter = new ColumnFilter(new String[]{origModelSettings.getString(COLUMN_SELECTION_KEY)});
            newSettings.m_unit =
                Granularity.fromString(origModelSettings.getString(DURATION_FIELD_KEY)).getTimeBasedGranularityUnit();
            newSettings.m_roundingBehaviour =
                DataTypeMode.Truncated.name().equals(origModelSettings.getString(DATE_TYPE_KEY))
                    ? RoundingBehaviour.INTEGER : RoundingBehaviour.DOUBLE;
            newSettings.m_appendOrReplaceColumn = ReplaceOrAppend.APPEND;
            newSettings.m_suffix = " (out)";
