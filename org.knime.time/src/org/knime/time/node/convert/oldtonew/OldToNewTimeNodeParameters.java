...
final class OldToNewTimeNodeParameters implements NodeParameters {
    ...
    @Section(title = "New Type Selection",
        description = "Configure how the new Date&amp;Time type is determined: either automatically from the first "
            + "non-missing row or manually by selecting the desired target type.")
    @After(ColumnSelectionSection.class)
    interface NewTypeSection {
    }

    @Section(title = "Time Zone Selection",
        description = "Control time zone enrichment. When automatic type detection is enabled you may request adding a "
            + "time zone if the legacy value contains both date and time. When selecting types manually a time zone is "
            + "required only for ZonedDateTime; in that case choose the zone below.")
    @After(NewTypeSection.class)
    ...
    enum ReplaceAppendMode {
            ...
            APPEND("Append selected columns"), @Label("Replace selected columns")
    ...
    @Widget(title = "Output mode",
        description = "Select how converted columns should appear in the output:"
            + " <b>Replace selected columns</b> overwrites the originals, while <b>Append selected columns</b>"
            + " keeps originals and adds new columns with a configurable suffix.")
    ...
    @Widget(title = "Suffix of appended columns",
        description = "Suffix appended to each converted column when <b>Append selected columns</b> is chosen. "
            + "Must yield unique column names. Ignored in replace mode.")
    @Layout(ReplaceAppendSection.class)
    ...
    @Widget(title = "Automatic type detection",
        description = "If enabled the new type for each included column is inferred "
            ...
            + "decide whether LocalDate, LocalTime, LocalDateTime or (optionally) ZonedDateTime is chosen.")
    ...
    @Widget(title = "Manual new type",
        description = "Choose the target Date&amp;Time type to which all included columns will be "
            + "converted when automatic type detection is disabled. Available options: LocalDate, LocalTime, "
    ...
    @Widget(title = "Add time zone, if possible",
        description = "When enabled or if Date&amp;Time is selected as a new type this option "
            + "attempts to upgrade legacy values containing both date and time components to "
            + "ZonedDateTime by attaching the selected time zone. If a value contains date only "
            + "or time only no time zone is added.")
    ...
    @Widget(title = "Time zone",
        description = "The IANA time zone identifier used either (a) when automatic type detection "
            ...
            + "and the chosen type is ZonedDateTime.")
    @Layout(TimeZoneSection.class)
    ...
    static final class TimeZoneChoices implements StringChoicesProvider {
        @Override
        public java.util.List<String> choices(final NodeParametersInput in) {
    ...
    static final class ReplaceAppendPersistor implements NodeParametersPersistor<ReplaceAppendMode> {
        @Override
        public void save(final ReplaceAppendMode obj, final org.knime.core.node.NodeSettingsWO settings) {
            settings.addString("replace_or_append", obj.legacy());
        }

        @Override
        public ReplaceAppendMode load(final org.knime.core.node.NodeSettingsRO settings)
    ...
    static final class NewTypeEnumPersistor implements NodeParametersPersistor<DateTimeType> {
        @Override
        public void save(final DateTimeType obj, final org.knime.core.node.NodeSettingsWO settings) {
            settings.addString("newTypeEnum", obj.name());
        }

        @Override
        public DateTimeType load(final org.knime.core.node.NodeSettingsRO settings)
            throws org.knime.core.node.InvalidSettingsException {
            return DateTimeType.valueOf(settings.getString("newTypeEnum", DateTimeType.LOCAL_DATE_TIME.name()));
        }

        @Override
        public String[][] getConfigPaths() {
            return new String[][]{{"newTypeEnum"}};
        }
    }
