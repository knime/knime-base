...
final class OldToNewTimeNodeParameters implements NodeParameters {
    ...
    @Widget(title = "Output mode",
        description = "Select how converted columns should appear in the output: <b>Replace selected columns</b> overwrites the originals, while <b>Append selected columns</b> keeps originals and adds new columns with a configurable suffix.")
    ...
    @Widget(title = "Add time zone, if possible",
        description = "When enabled or if Date&amp;Time is selected as a new type this option attempts to upgrade legacy values containing both date and time components to ZonedDateTime by attaching the selected time zone. If a value contains date only or time only no time zone is added.")
    ...
    OldToNewTimeNodeParameters(final NodeParametersInput context) {
    }
    ...
    static final class TimeZoneChoices implements StringChoicesProvider { // was ChoicesProvider (annotation) -> implement provider interface
        @Override
        public java.util.List<String> choices(final org.knime.node.parameters.NodeParametersInput in) {
