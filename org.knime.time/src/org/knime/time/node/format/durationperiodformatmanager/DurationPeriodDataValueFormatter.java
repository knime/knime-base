...
public final class DurationPeriodDataValueFormatter implements ValueFormatModel {
    ...
    private final FormatterAlignment m_alignment;

    DurationPeriodDataValueFormatter(final DurationPeriodStringFormat format, final FormatterAlignment alignment) {
        m_format = format;
    ...
    public String getPlaintext(final DataValue dataValue) {
        ...
        if (dataValue instanceof DurationValue dv) {
            ...
            throw new IllegalArgumentException(
                "DataValue must be either a DurationValue or a PeriodValue for this formatter");
        }
    ...
    public String getHTML(final DataValue dataValue) {
        ...
        return "<span style=\"%s\">%s</span>".formatted( //
            convertAlignmentToCssStyle(m_alignment), //
            plainText //
        );
    }
    ...
    public void save(final ConfigWO config) {
        ...
        config.addString(CFG_KEY_ALIGNMENT, m_alignment.name());
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hashCode(m_format, m_alignment);
    }
    ...
    private static String convertAlignmentToCssStyle(final FormatterAlignment alignment) {
        return "display:inline-block;width:100%%;text-align:%s".formatted(alignment.name().toLowerCase(Locale.ROOT));
    }
    ...
    public static final class Factory implements ValueFormatModelFactory<DurationPeriodDataValueFormatter> {
        ...
        public DurationPeriodDataValueFormatter getFormatter(final ConfigRO config) throws InvalidSettingsException {
            ...
            var alignmentAsString = config.getString(CFG_KEY_ALIGNMENT);
            FormatterAlignment alignment;
            try {
                alignment = FormatterAlignment.valueOf(alignmentAsString);
            } catch (IllegalArgumentException e) {
                throw new InvalidSettingsException("Invalid alignment: '%s'. Valid alignments are: %s".formatted( //
                    alignmentAsString, //
                    Arrays.stream(FormatterAlignment.values()) //
                        .map(FormatterAlignment::name) //
                        .collect(Collectors.joining(", ")) //
                ), e);
            }
