...
final class DateTimeFormatManagerNodeModel extends WebUINodeModel<DateTimeFormatManagerNodeSettings> {
    ...
    private static DataTableSpec attachFormatterToSelectedColumns(final DataTableSpec spec,
        ...
        var handler = new ValueFormatHandler(new DateTimeDataValueFormatter( //
            settings.m_format, //
            LocaleUtils.toLocale(settings.m_locale)) //
        );

        var tableSpecCreator = new DataTableSpecCreator(spec);
        var targetColumns = settings.m_columnFilter
            .getSelected(getCompatibleColumns(spec, DateTimeFormatManagerNodeSettings.DATE_TIME_COLUMN_TYPES), spec);

        for (var columnName : targetColumns) {
            var columnSpec = spec.getColumnSpec(columnName);
    ...
    private static String[] getCompatibleColumns(final DataTableSpec spec,
    ...
    private static boolean isDateTimeColumn(final DataColumnSpec columnSpec) {
