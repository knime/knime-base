...
final class TimeShiftNodeModel extends WebUINodeModel<TimeShiftNodeSettings> {

    /**
     * @param configuration
     */
    protected TimeShiftNodeModel(final WebUINodeConfiguration configuration) {
        super(configuration, TimeShiftNodeSettings.class);
    }

    @Override
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs, final TimeShiftNodeSettings settings)
        throws InvalidSettingsException {

        switch (settings.m_shiftMode) {
            case DURATION_COLUMN:
                DateTimeShiftSettingsUtils.validateDurationColumn(settings.m_durationColumn, inSpecs[0]);
                break;
    ...
    protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec,
        final TimeShiftNodeSettings modelSettings) throws Exception {//NOSONAR
    ...
    private ColumnRearranger createColumnRearranger(final DataTableSpec spec, final TimeShiftNodeSettings settings) {

        var shiftNodeSettings = new ShiftContext.Builder() //
            .temporalColumn(settings.m_durationColumn) //
            .temporalValue(settings.m_shiftDurationValue) //
        ...
        return switch (settings.m_shiftMode) {
            case DURATION_COLUMN -> DateTimeShiftUtils.createColumnRearranger(spec, shiftNodeSettings,
