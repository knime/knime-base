...
public class DateTimeDifferenceNodeModel2 extends WebUINodeModel<DateTimeDifferenceNodeSettings> {
    ...
    private static class DifferenceCellFactory extends SingleCellFactory {
        ...
        public DataCell getCell(final DataRow row) {
            var firstValueAsTemporal = extractTemporalFromDataCell(row.getCell(m_firstColumnIndex));
            var secondValueAsTemporal = getSecondValueAsTemporal(row, m_inSpec, m_settings);

            return createDifferenceCell(firstValueAsTemporal, secondValueAsTemporal, m_settings);
        }

        private static Temporal getSecondValueAsTemporal(final DataRow row, final DataTableSpec inSpec,
            final DateTimeDifferenceNodeSettings settings) {

            return switch (settings.m_secondDateTimeValueType) {
                case COLUMN -> {
                    var secondColumnIndex = inSpec.findColumnIndex(settings.m_secondColumnSelection.m_selected);
                    yield extractTemporalFromDataCell(row.getCell(secondColumnIndex));
                }
                case FIXED_DATE_TIME -> createTemporalFromPotentiallyNullFields( //
                    settings.m_localDateFixed, //
                    settings.m_localTimeFixed, //
                    settings.m_timezoneFixed //
                    );
                case EXECUTION_DATE_TIME -> ZonedDateTime.now();
                default -> throw new IllegalArgumentException(
                    "Unsupported SecondDateTimeValueType: " + settings.m_secondDateTimeValueType);
            };
        }
    ...
    private static BufferedDataTable createRowDifferenceTable(final BufferedDataTable inData,
        ...
        try (var inDataIterator = inData.iterator()) {
            ...
            while (inDataIterator.hasNext()) {
                ...
                var difference = createDifferenceCell( //
                    extractTemporalFromDataCell(previousRow.getCell(firstColumnIndex)), //
                    extractTemporalFromDataCell(currentRow.getCell(firstColumnIndex)), //
                    settings //
                );
