...
public final class SettingsDataUtil {
    ...
    public static Optional<DataTable> getDataTable(final DefaultNodeSettingsContext context) {
        Optional<DataTable> dt = Optional.empty();
        var inputPorts = context.getInputPortObjects();

        if (inputPorts != null) {
            dt = context.getDataTable(0);
        }
        return dt;
    }
     ...
     * @return the index of the column; returns -1 if no valid columns found
     */
    public static int getFirstColumnIndexFromSelectedColumnArray(final DataTableSpec spec,
        ...
        var firstCompatibleColumnNameOptional = spec.stream() //
            .filter(columnFilter) // compatible with DurationValue or PeriodValue?
            .map(DataColumnSpec::getName) // map to column name
            .filter(Arrays.asList(selectedColumns)::contains) // selected in the twinlist?
            .findFirst();
     ...
     * @return the cell; or null if no non-missing cell found
     */
    public static DataCell getFirstNonMissingCellInColumn(final DataTable dt, final int colNum) {
        for (var row : dt) {
