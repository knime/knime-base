...
final class DurationPeriodToStringNodeSettingsTest extends DefaultNodeSettingsSnapshotTest { // NOSONAR
    ...
    static final PortObjectSpec[] TEST_TABLE_SPECS = new PortObjectSpec[]{new DataTableSpec( //
        new String[]{"periodTest", "durationTest"}, new DataType[]{ //
            DataType.getType(PeriodCell.class), //
            DataType.getType(DurationCell.class)} //
            )};

    protected DurationPeriodToStringNodeSettingsTest() {
        super(getConfig());
    }
    ...
    private static SnapshotTestConfiguration getConfig() {
        return SnapshotTestConfiguration.builder() //
            ...
            .testJsonFormsForModel(DurationPeriodToStringNodeSettings.class) //
            .testJsonFormsWithInstance(SettingsType.MODEL, () -> readSettings()) //
            .testNodeSettingsStructure(() -> readSettings()) //
            .build();
    }

    private static DurationPeriodToStringNodeSettings readSettings() {
        try {
            var path = getSnapshotPath(DurationPeriodToStringNodeSettings.class).getParent().resolve("node_settings")
                .resolve("DurationPeriodToStringNodeSettings.xml");
            try (var fis = new FileInputStream(path.toFile())) {
                var nodeSettings = NodeSettings.loadFromXML(fis);
                return NodeParametersUtil.loadSettings(nodeSettings.getNodeSettings(SettingsType.MODEL.getConfigKey()),
                    DurationPeriodToStringNodeSettings.class);
            }
