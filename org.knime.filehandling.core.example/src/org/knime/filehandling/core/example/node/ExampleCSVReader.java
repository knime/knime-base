 ...
 * 
 * Here we can manipulate the spec of the reader.
 * In this case we read the first row of the CSV to count the number of columns.
 *
...
final class ExampleCSVReader implements TableReader<ExampleCSVReaderConfig, Class<?>, String> {
    ...
    /**
     * Column names are generated with Column prefix
     * All the columns are String and has type
     */
    @Override
    public TypedReaderTableSpec<Class<?>> readSpec(final FSPath path,
        ...
        ArrayList<Boolean> hasTypes = new ArrayList<>();
        try (final ExampleCSVRead read = new ExampleCSVRead(path, config)) {
            final RandomAccessible<String> columns = read.next();
            int i = 0;
            for (String string : columns) {
                colNames.add(String.format("Column%x", i));
                colTypes.add(String.class);
                hasTypes.add(Boolean.TRUE);
                i++;
            }
            return TypedReaderTableSpec.create(colNames, colTypes, hasTypes);
        }
