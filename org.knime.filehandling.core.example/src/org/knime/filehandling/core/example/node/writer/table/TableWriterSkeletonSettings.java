...
final class TableWriterSkeletonSettings {
	...
	/**
	 * The extensions that are selectable when browsing the file system and will be
	 * automatically appended when entering the file name via the browser.
	 */
	...
	TableWriterSkeletonSettings(final PortsConfiguration portsCfg, final String fsPortId) {
		...
		final EnumConfig<FileOverwritePolicy> overwritePolicies = EnumConfig.create(FileOverwritePolicy.FAIL,
	...
	void saveInModel(NodeSettingsWO settings) {
		m_writerFileChooser.saveSettingsTo(settings);
	}

	void validateInModel(NodeSettingsRO settings) throws InvalidSettingsException {
		m_writerFileChooser.validateSettings(settings);
	}

	void loadInModel(NodeSettingsRO settings) throws InvalidSettingsException {
