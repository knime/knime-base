...
final class TableWriterSkeletonNodeModel extends NodeModel {
	...
	protected PortObject[] execute(PortObject[] inObjects, ExecutionContext exec) throws Exception {
		...
		try (final WritePathAccessor accessor = m_settings.getWriterFileChooser().createWritePathAccessor()) {
			...
			try (final OutputStream os = FSFiles.newOutputStream(outputPath,
	...
	private void checkOverwrite(final FSPath outpath) throws AccessDeniedException, IOException {
		final FileOverwritePolicy fileOverwritePolicy = m_settings.getWriterFileChooser().getFileOverwritePolicy();
