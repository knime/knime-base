...
public final class TransferFilesTableNodeFactory extends AbstractTransferFilesNodeFactory<TransferFilesTableNodeModel> implements NodeDialogFactory, KaiNodeInterfaceFactory {
    ...
    private static final String FULL_DESCRIPTION = """
            <p> This node copies or moves files from a source (folder or file) to another folder. The node offers
                options to either specify the destination based on a column in the input table or via a file chooser. If
                the "Delete source files (move)" option is checked the node performs a move operation for which the
                source files will be deleted after the copying process is done. </p> <p> <i>This node can access a
