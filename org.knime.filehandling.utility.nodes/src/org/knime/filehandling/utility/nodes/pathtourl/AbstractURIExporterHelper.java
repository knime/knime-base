 ...
 */
public abstract class AbstractURIExporterHelper {
    ...
    void setPortObjectSpecs(final PortObjectSpec[] specs) throws InvalidSettingsException {
    ...
    protected DataColumnSpec getPathColumnSpec() {
        return ((DataTableSpec)m_portObjectSpecs[m_dataTablePortIndex])
            .getColumnSpec(m_selectedColumn.getStringValue());
    }
    ...
    protected void validatePathColumn() throws InvalidSettingsException {
        final DataColumnSpec pathColSpec = getPathColumnSpec();
        CheckUtils.checkSetting(pathColSpec != null, "Input table does not contain selected column %s",
            m_selectedColumn.getStringValue());
        FSLocationColumnUtils.validateFSLocationColumn(pathColSpec, getFileSystemPortObjectSpec());
    }
    ...
    abstract void loadSettingsFrom(final NodeSettingsRO exporterConfig);
}
