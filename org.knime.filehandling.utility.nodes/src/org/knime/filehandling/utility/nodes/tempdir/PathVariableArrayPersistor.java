...
import org.knime.filehandling.utility.nodes.dialog.variables.FSLocationVariables;
import org.knime.filehandling.utility.nodes.tempdir.CreateTempDir2NodeParameters.PathVariable;
import org.knime.node.parameters.persistence.NodeParametersPersistor;

/**
 * Custom persistor for PathVariable arrays that need backwards compatibility with FSLocationVariableTableModel. This
 * persistor converts between PathVariable[] and the legacy format using separate arrays for names, paths, and
 * extensions.
 *
 * @author AI Migration Pipeline v1.2
 */
final class PathVariableArrayPersistor implements NodeParametersPersistor<PathVariable[]> {

    @Override
    public PathVariable[] load(final NodeSettingsRO settings) throws InvalidSettingsException {
        FSLocationVariables variables;
        if (settings.containsKey(CreateTempDir2NodeConfig.CFG_ADDITIONAL_PATH_VARIABLES)) {
            final var varSettings = settings.getNodeSettings(CreateTempDir2NodeConfig.CFG_ADDITIONAL_PATH_VARIABLES);
    ...
    public void save(final CreateTempDir2NodeParameters.PathVariable[] pathVariables, final NodeSettingsWO settings) {
        ...
        final var varSettings = settings.addNodeSettings(CreateTempDir2NodeConfig.CFG_ADDITIONAL_PATH_VARIABLES);
        new FSLocationVariables(names, paths, extensions).save(varSettings);
    }

    @Override
    public String[][] getConfigPaths() {
        return new String[][]{{CreateTempDir2NodeConfig.CFG_ADDITIONAL_PATH_VARIABLES, "variable_names"},
            {CreateTempDir2NodeConfig.CFG_ADDITIONAL_PATH_VARIABLES, "path_values"},
            {CreateTempDir2NodeConfig.CFG_ADDITIONAL_PATH_VARIABLES, "file_extensions"}};
    }
}
