...
class SetPosixPermissionsNodeParameters implements NodeParameters {
    ...
    Permissions[] m_permissions = new Permissions[2];
    ...
    static final class PermissionsArrayProvider implements StateProvider<Permissions[]> {
        ...
        public Permissions[] computeState(final NodeParametersInput parametersInput)
            ...
            final var currentPermissions = m_permissionArraySupplier.get();
            ...
            final var permissions = Arrays.stream(currentPermissions)
                    .filter(Objects::nonNull).toArray(Permissions[]::new);
            if (permissions.length != 3) {
                throw new StateComputationFailureException();
            }

            return new Permissions[]{
                new Permissions(PermissionRole.OWNER, permissions[0].m_read,
                    currentPermissions[0].m_write, permissions[0].m_execute),
                new Permissions(PermissionRole.GROUP, permissions[1].m_read,
                    currentPermissions[1].m_write, permissions[1].m_execute),
                new Permissions(PermissionRole.OTHERS, permissions[2].m_read,
                    currentPermissions[2].m_write, permissions[2].m_execute)};
        }
    ...
    static final class PermissionsArrayPersistor implements NodeParametersPersistor<Permissions[]> {
        ...
        public void save(final Permissions[] param, final NodeSettingsWO settings) {
            if (param == null || param.length == 0) {
                settings.addStringArray(SetPosixPermissionsNodeSettings.KEY_PERMISSIONS, new String[0]);
            } else {
                final var permissions = Arrays.stream(param).filter(Objects::nonNull).toArray(Permissions[]::new);
                if (permissions.length != 3) {
                    settings.addStringArray(SetPosixPermissionsNodeSettings.KEY_PERMISSIONS, new String[0]);
                } else {
                    var ownerPermissions = getPermissions(param[0], new PosixFilePermission[]{
                        PosixFilePermission.OWNER_READ,
                        PosixFilePermission.OWNER_WRITE,
                        PosixFilePermission.OWNER_EXECUTE});
                    var groupPermissions = getPermissions(param[1], new PosixFilePermission[]{
                        PosixFilePermission.GROUP_READ,
                        PosixFilePermission.GROUP_WRITE,
                        PosixFilePermission.GROUP_EXECUTE});
                    var othersPermissions = getPermissions(param[2], new PosixFilePermission[]{
