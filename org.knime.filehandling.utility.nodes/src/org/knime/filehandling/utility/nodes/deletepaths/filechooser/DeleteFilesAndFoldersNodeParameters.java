 ...
 *
 * History
 *   26 Nov 2025 (Thomas Reifenberger, TNG Technology Consulting GmbH): created
 *
 */
...
/**
 * Node parameters for Delete Files/Folders.
 *
...
class DeleteFilesAndFoldersNodeParameters implements NodeParameters {
    ...
    @Persist(configKey = DeleteFilesAndFoldersNodeConfig.CFG_FILE_CHOOSER)
    ...
    LegacyMultiFileSelection m_fileChooser = new LegacyMultiFileSelection(MultiFileSelectionMode.FILE);

    @Persist(configKey = CFG_FAIL_IF_DELETE_FAILS)
    @Migration(FileIfDeleteFailsMigration.class)
    @Widget(title = "Fail if delete fails", description = """
            If this option is checked, the node will fail if one of the files could not be deleted. If it is
            unchecked the output table will contain a column that indicates if a file/folder got successfully
            deleted or not.
            """)
     ...
     */
    private static final class FileIfDeleteFailsMigration implements NodeParametersMigration<Boolean> {
        ...
        public List<ConfigMigration<Boolean>> getConfigMigrations() {
            return List.of( //
                ConfigMigration.builder(FileIfDeleteFailsMigration::loadFromLegacyKey)
