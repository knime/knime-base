...
final class DecompressNodeModel extends NodeModel {
    ...
    protected PortObject[] execute(final PortObject[] inData, final ExecutionContext exec) throws Exception {
        ...
        try (final ReadPathAccessor readAccessor = m_config.getInputFileChooserModel().createReadPathAccessor();
                final WritePathAccessor writeAccessor = m_config.getOutputDirChooserModel().createWritePathAccessor()) {
            final FSPath sourcePath = readAccessor.getRootPath(m_statusConsumer);
            final FSPath targetPath = writeAccessor.getOutputPath(m_statusConsumer);
            m_statusConsumer.setWarningsIfRequired(this::setWarningMessage);
            final Decompressor decompressor = new Decompressor(m_config, rowOutput, exec);

            decompressor.decompress(sourcePath, targetPath);
        } finally {
            rowOutput.close();
        }
