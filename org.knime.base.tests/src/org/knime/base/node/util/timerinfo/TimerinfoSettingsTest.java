...
/**
 * Settings test for {@code TimerinfoNodeSettings}.
 *
 * @author Magnus Gohm, KNIME AG, Konstanz, Germany
 */
@SuppressWarnings("restriction") // webui
public class TimerinfoSettingsTest extends DefaultNodeSettingsSnapshotTest{
    ...
    private static TimerinfoNodeSettings readSettings() {
        try {
            var path = getSnapshotPath(TimerinfoNodeSettings.class).getParent().resolve("node_settings")
                .resolve("TimerInfoNodeSettings.xml");
            try (var fis = new FileInputStream(path.toFile())) {
                var nodeSettings = NodeSettings.loadFromXML(fis);
                return DefaultNodeSettings.loadSettings(nodeSettings.getNodeSettings(SettingsType.MODEL.getConfigKey()),
                    TimerinfoNodeSettings.class);
            }
