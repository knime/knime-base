...
class GroupByNodeParametersTest extends DefaultNodeSettingsSnapshotTest {
    ...
    private static SnapshotTestConfiguration getConfig() {
        return SnapshotTestConfiguration.builder() //
            ...
            .testJsonFormsForModel(GroupByNodeParameters.class) //
            .testJsonFormsWithInstance(SettingsType.MODEL, () -> readSettings()) //
            .testNodeSettingsStructure(() -> readSettings()) //
            .build();
    }

    private static GroupByNodeParameters readSettings() {
        try {
            var path = getSnapshotPath(GroupByNodeParameters.class).getParent().resolve("node_settings")
                .resolve("GroupByNodeParameters.xml");
            try (var fis = new FileInputStream(path.toFile())) {
                var nodeSettings = NodeSettings.loadFromXML(fis);
                return NodeParametersUtil.loadSettings(nodeSettings.getNodeSettings(SettingsType.MODEL.getConfigKey()),
                    GroupByNodeParameters.class);
            }
