...
public abstract class AbstractMultipleDateTimeOperandsValidationTest {
    ...
    public void canCheckDateValues() {
        ...
        ValidationResult result = m_validation
            .apply(new OperatorParameters(columnSpec, m_operator, new String[]{"2017-05-23", "2019dfsdsf-01-15"}));
        ...
        Assert.assertThat(result.getErrors().get(1).getError(), Matchers.containsString("convert to local date"));
    }
    ...
    public void canCheckDateTimeValues() {
        ...
        Assert.assertTrue("no errors", result.hasErrors());
        Assert.assertThat(result.getErrors(), Matchers.hasSize(2));
        Assert.assertThat(result.getErrors().get(0).getError(), Matchers.containsString("convert to local date & time"));
        Assert.assertThat(result.getErrors().get(1).getError(), Matchers.containsString("convert to local date & time"));
    }
    ...
    public void canCheckTimeValue() {
        ...
        Assert.assertTrue("no errors", result.hasErrors());
        Assert.assertThat(result.getErrors(), Matchers.hasSize(2));
        Assert.assertThat(result.getErrors().get(0).getError(), Matchers.containsString("convert to local time"));
        Assert.assertThat(result.getErrors().get(1).getError(), Matchers.containsString("convert to local time"));
    }
}
